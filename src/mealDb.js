const recipes = [
  {
    id: 2,
    mealName: "Nigerian Turkey Stew",
    overview:
      "This delicious, protein-packed Nigerian turkey stew is made with ground peppers, tomatoes, turkey parts, and spices. It will have you licking your lips and drooling!",
    image:
      "https://lowcarbafrica.com/wp-content/uploads/2021/09/Nigerian-Turkey-Stew-IG-1.jpg",

    recipe: {
      instructions: [
        "1. First of all, set your oven to 400F.",
        "2. Next, wash and drain the turkey, then rub it all over with salt, pepper, and olive oil.",
        "3. Bake the turkey in the oven for 30 minutes, flipping it halfway.",
        "4. Alternatively, you can also air fry the turkey for 20-30 minutes, flipping it halfway. This is my preferred way because it's so much faster.",
        "5. Chop the onions and set them aside. Blend the tomatoes and peppers with very little water.",
        "6. Sauté the chopped onions in olive oil on medium heat.",
        "7. Add the blended pepper and spices, and simmer for 10 minutes.",
        "8. Add the turkey, chicken stock, and then stir it together.",
        "9. Depending on how thick it is, add a little more broth till it reaches a stew-like consistency.",
        "10. Taste for salt and add more if you need.",
        "11. Cover and let simmer for 10 more minutes on medium-low heat.",
      ],
      ingredients: [
        {
          name: "2 lbs turkey meat",
          image:
            "https://www.lafoodservice.com/wp-content/uploads/2020/06/turkey-breast-sliced-jennie-o.jpg",
        },
        {
          name: "3 plum tomatoes",
          image:
            "https://d2j6dbq0eux0bg.cloudfront.net/images/26774020/1363244751.jpg",
        },
        {
          name: "1 habanero pepper",
          image:
            "https://www.livingseedcompany.com/cdn/shop/products/habanero-pepper-capsicum-chinense-694660.png?v=1702656028",
        },
        {
          name: "1 red bell pepper",
          image:
            "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
        },
        {
          name: "½ onion",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "¼ cup olive oil",
          image:
            "https://content.instructables.com/F5R/Y0H0/IHVVPRS6/F5RY0H0IHVVPRS6.jpg?auto=webp&frame=1&width=320&md=a3d3b3b73ae688ac87e75fbee681be91",
        },
        {
          name: "1 teaspoon bouillon powder",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "½ teaspoon curry",
          image:
            "https://thumbs.dreamstime.com/z/teaspoon-curry-powder-isolated-white-background-261886802.jpg",
        },
        {
          name: "½ teaspoon thyme",
          image:
            "https://rachelsspicecompany.com/wp-content/uploads/2018/03/ThymeLeaf.jpg",
        },
        {
          name: "½ teaspoon black pepper",
          image:
            "https://as1.ftcdn.net/v2/jpg/02/79/24/98/1000_F_279249862_iO0LUG8l16W0vvW3Aw16TYCxLuUwTAk2.jpg",
        },
        {
          name: "½ cup chicken stock",
          image:
            "https://media01.stockfood.com/largepreviews/MzQyMTQzNDY2/11036886-2-1-2-Cups-of-Chicken-Broth-in-a-Measuring-Cup.jpg",
        },
        {
          name: "salt to taste",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
      ],
    },
    course: "Main",
    prepTime: "5 minutes",
    cookTime: "50 minutes",
    totalTime: "55 minutes",
    prepLevel: "Med",
    nutritionalFacts: {
      calories: 203,
      servings: 8,
      fat: "6g",
      transFat: "1g",
      protein: "26g",
      carbs: "3g",
      SaturatedFat: "2g",
      PolyunsaturatedFat: "1g",
      MonounsaturatedFat: "6g",
      Cholesterol: "76mg",
      Sodium: "222mg",
      Potassium: "376mg",
      Fiber: "1g",
      Sugar: "2g",
      VitaminA: "758IU",
      VitaminC: "24mg",
      Calcium: "18mg",
      Iron: "1mg",
    },
    originalSource: "https://lowcarbafrica.com/nigerian-turkey-stew/",
  },

  {
    id: 37,
    mealName: "Green Detox Keto Smoothie",
    overview:
      "Revitalize your day with this Green Detox Keto Smoothie. Packed with the goodness of kale, fresh mint, avocado, fennel, and more, this unsweetened, low-carb smoothie is a refreshing treat for your body and taste buds.",
    image:
      "https://www.sugarfreemom.com/wp-content/uploads/2016/06/green-tea-detox-smoothie1-1-of-1.jpg",
    recipe: {
      instructions: [
        "1. Scoop the avocado flesh out and place all ingredients into a high-speed blender.",
        "2. Blend until smooth.",
        "3. Adjust thickness by adding more almond milk if desired.",
      ],
      ingredients: [
        {
          name: "Handful / 15g kale",
          image:
            "https://www.shutterstock.com/image-photo/bunch-organic-kale-on-rustic-600nw-291502316.jpg",
        },
        {
          name: "Handful / 10g fresh mint leaves, picked",
          image:
            "https://plantura.garden/uk/wp-content/uploads/sites/2/2021/04/store-fresh-mint.jpg",
        },
        {
          name: "½ avocado",
          image:
            "https://media.post.rvohealth.io/wp-content/uploads/sites/3/2021/07/Stocksy_txpa387b205vt9300_Medium_3358820_thumb-732x549.jpg",
        },
        {
          name: "¼ / 50g fennel or 1 celery stick",
          image:
            "https://www.dadcooksdinner.com/wp-content/uploads/2020/03/Stalk-of-Celery-vs-Rib-of-Celery-2020-03-03-Celery-Stalk-vs-Rib0391-1920x.jpg",
        },
        {
          name: "¼ small cucumber (100g)",
          image:
            "https://sa1s3optim.patientpop.com/assets/images/provider/photos/2445929.png",
        },
        {
          name: "1 tablespoon protein powder unsweetened",
          image:
            "https://ng.jumia.is/unsafe/fit-in/500x500/filters:fill(white)/product/67/5766032/1.jpg?0069",
        },
        {
          name: "1 cup / 240 ml almond milk unsweetened",
          image:
            "https://navs-online.org/wp-content/uploads/2016/03/Almond-Milk-v2.jpg",
        },
        {
          name: "1 tablespoon almond butter",
          image:
            "https://hungryhealthyhappy.com/wp-content/uploads/2018/02/Vanilla-and-Honey-Almond-Butter-featured.jpg",
        },
        {
          name: "½ lemon, juiced",
          image:
            "https://hungryhealthyhappy.com/wp-content/uploads/2018/02/Vanilla-and-Honey-Almond-Butter-featured.jpg",
        },
      ],
    },
    course: "Beverage",
    prepTime: "5 minutes",
    totalTime: "5 minutes",
    prepLevel: "Easy",
    servings: 1,
    nutritionalFacts: {
      calories: 137,
      totalCarbohydrates: "7.7g",
      protein: "5.6g",
      fat: "10g",
      fiber: "4.7g",
      sugar: "1.7g",
    },
    originalSource: "https://sugarfreelondoner.com/keto-green-smoothie/",
  },
  {
    id: 1,
    mealName: "Keto Jollof Cauliflower Rice",
    overview:
      "A low-carb version of the classic Nigerian Jollof Rice, using cauliflower rice for a keto-friendly twist.",
    image:
      "https://divaliciousrecipes.com/wp-content/uploads/2016/08/Cauliflower-Rice.jpg",

    recipe: {
      instructions: [
        "1. Cut the tomatoes, peppers and onions into large chunks.",
        "2. Place in a blender and add half a cup of water.",
        "3. Blend the veggies, pour in a pot and put on the stove on high heat for about 10 minutes.",
        "4. In a separate skillet, pour in palm oil (or olive oil).",
        "5. When heated, add the blended tomato and peppers.",
        "6. Add spices and stir. Leave on medium heat for 10 minutes.",
        "7. Add the cauliflower rice and stir.",
        "8. If using fresh cauliflower rice, add half a cup of water or stock. Omit if using frozen cauliflower rice.",
        "9. Cover the pot for a few minutes.",
        "10. Stir and taste for salt. Adjust accordingly.",
        "11. Serve & enjoy!",
      ],
      ingredients: [
        {
          name: "1 lb cauliflower rice or one head of cauliflower",
          image:
            "https://bombayspiceland.com/wp-content/uploads/2020/07/cauliflower.jpg",
        },
        {
          name: "1 tomato",
          image:
            "https://hahtolanliha.fi/wp-content/uploads/2016/03/Tomato.jpg",
        },
        {
          name: "½ habanero pepper or ¼ teaspoon cayenne pepper",
          image:
            "https://media.istockphoto.com/id/1164717353/photo/red-hot-habanero-peppers-isolated-on-white-background.jpg?s=612x612&w=0&k=20&c=Iy3JP2FSS6aMapD6ZDan6Y10zVorZTUUhz_7Mebt_oM=",
        },
        {
          name: "1 red bell pepper",
          image:
            "https://i0.wp.com/tamhouston.com/wp-content/uploads/2022/03/red-bell-pepper-single.jpg?fit=1080%2C1080&ssl=1",
        },
        {
          name: "½ onion",
          image:
            "https://media.istockphoto.com/id/175448479/photo/red-onion-slice.jpg?s=612x612&w=0&k=20&c=LN1FqmYnwbHkhgBxV9FQ9zFm52iJKo1gO1EwNjbKuAw=",
        },
        {
          name: "¼ cup palm oil or olive oil",
          image:
            "https://martking.ng/wp-content/uploads/2022/08/Martking-Online-Store-Palm-oil-1.5L.jpg",
        },
        {
          name: "½ teaspoon salt",
          image:
            "https://www.shutterstock.com/shutterstock/videos/1023011272/thumb/1.jpg?ip=x480",
        },
        {
          name: "1 teaspoon bouillon powder",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "1 tablespoon ground crayfish optional",
          image:
            "https://ekoaketefoods.com/cdn/shop/products/blended-crayfishmain.png?v=1622236106",
        },
        {
          name: "½ teaspoon black pepper",
          image:
            "https://images.eatthismuch.com/img/127788_Shamarie84_60d5dcc1-cfa5-42f8-8c99-198e31473df4.jpeg",
        },
        {
          name: "½ cup water or stock",
          image:
            "https://img-cdn.inc.com/image/upload/w_1024,h_576,c_fill/images/panoramic/getty_1283524975_sgbp6n.jpg",
        },
      ],
    },
    course: "Main",
    prepTime: "5 minutes",
    cookTime: "15 minutes",
    totalTime: "20 minutes",
    prepLevel: "Easy",
    nutritionalFacts: {
      calories: 70,
      servings: 4,
      fat: "5g",
      protein: "1g",
      carbs: "5g",
      SaturatedFat: "1g",
      PolyunsaturatedFat: "1g",
      MonounsaturatedFat: "1g",
      Cholesterol: "1mg",
      Sodium: "1094mg",
      Potassium: "179mg",
      Fiber: "2g",
      Sugar: "3g",
      VitaminA: "945IU",
      VitaminC: "57mg",
      Calcium: "12mg",
      Iron: "1mg",
    },
    originalSource:
      "https://lowcarbafrica.com/nigerian-cauliflower-jollof-rice/",
  },

  {
    id: 4,
    mealName: "Cabbage Rice",
    overview:
      "Cabbage rice (also known as riced cabbage) is a quick, easy, and savory side dish and is a great alternative to cauliflower rice with only 7 net carbs! You can easily double it up for a full, satisfying, and nutritious meal.",
    image:
      "https://thymeandlove.com/wp-content/uploads/2021/09/vegan-fried-rice-720x720.jpg",

    recipe: {
      instructions: [
        "1. Wash the cabbage and pat dry.",
        "2. Chop the cabbage and place in the food processor.",
        "3. Use the chop function to shred it into smaller pieces.",
        "4. Chop the red bell pepper and scallions into small pieces.",
        "5. Heat the olive oil in a large skillet, then add the bell pepper and scallions and stir for a minute or two on medium-low heat.",
        "6. Add the cabbage, salt, black pepper, and bouillon powder and stir till it begins to soften.",
        "7. Stir and leave on medium heat for 5 minutes.",
        "8. Serve & enjoy!",
      ],
      ingredients: [
        {
          name: "½ cabbage medium",
          image: "https://ladgroup.org/wp-content/uploads/2020/12/p5.jpg",
        },
        {
          name: "2 tablespoon olive oil",
          image:
            "https://content.instructables.com/F5R/Y0H0/IHVVPRS6/F5RY0H0IHVVPRS6.jpg?auto=webp&frame=1&width=320&md=a3d3b3b73ae688ac87e75fbee681be91",
        },
        {
          name: "1 red bell pepper",
          image:
            "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
        },
        {
          name: "1 bunch scallion",
          image:
            "https://upload.wikimedia.org/wikipedia/commons/d/da/CSA-Red-Spring-Onions.jpg",
        },
        {
          name: "½ tablespoon bouillon powder",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "½ teaspoon salt to taste",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
        {
          name: "1 teaspoon black pepper",
          image:
            "https://as1.ftcdn.net/v2/jpg/02/79/24/98/1000_F_279249862_iO0LUG8l16W0vvW3Aw16TYCxLuUwTAk2.jpg",
        },
      ],
    },
    course: "Snack, Side Dishes and Appetizer",
    prepTime: "10 minutes",
    cookTime: "10 minutes",
    totalTime: "20 minutes",
    prepLevel: "Easy",
    nutritionalFacts: {
      calories: 177,
      servings: 3,
      fat: "14g",
      protein: "3g",
      carbs: "12g",
      SaturatedFat: "2g",
      PolyunsaturatedFat: "2g",
      MonounsaturatedFat: "10g",
      Cholesterol: "1mg",
      Sodium: "634mg",
      Potassium: "365mg",
      Fiber: "5g",
      Sugar: "7g",
      VitaminA: "1434IU",
      VitaminC: "107mg",
      Calcium: "70mg",
      Iron: "1mg",
    },
    originalSource:
      "https://lowcarbafrica.com/riced-cabbage-low-carb-side-dish/",
  },

  {
    id: 3,
    mealName: "Croaker Fish (Nigerian Peppered Fish)",
    overview:
      "This Nigerian peppered croaker fish recipe is a special dish made with just a few aromatic spices that blend together perfectly for a mouthwatering experience!",
    image:
      "https://lowcarbafrica.com/wp-content/uploads/2022/03/Peppered-Croaker-FIsh-3.webp",

    recipe: {
      instructions: [
        "1. Wash and pat the fish dry with a paper towel.",
        "2. Chop the red bell pepper and the onions into rings.",
        "3. Sprinkle the fresh fish with 1 teaspoon black pepper and ½ teaspoon salt and rub it all over the fish.",
        "4. Add some oil to a large frying pan on medium-high heat.",
        "5. Once heated, place fish in the oil, skin side down, and fry for about 10 minutes on medium heat till it is golden brown. Flip the fish a few times to make sure it is fully cooked.",
        "6. Take out the fried fish and place it on a plate lined with paper towels to absorb the extra oil.",
        "7. In a separate pan, add 2 tablespoons of olive oil. When heated, sauté the onions in the oil for a few minutes till translucent.",
        "8. Pour in the tomato sauce and add 1 teaspoon bouillon, 1 teaspoon black pepper, ½ teaspoon salt, and stir. Add the bouillon first and taste for salt before adding any.",
        "9. Simmer for 5 minutes, then add the chopped red bell pepper and half a cup of water.",
        "10. With the pot covered, cook for 5 minutes on medium-low heat, till the red bell pepper is soft.",
        "11. Pour in the fried fish and stir. Simmer for a few minutes on low heat till the fish absorbs the flavors.",
      ],
      ingredients: [
        {
          name: "1.5 lbs croaker fish",
          image:
            "https://www.sandfmeatshop.com.ng/wp-content/uploads/2021/10/GetPaidStock.com-614da5d584163.jpg",
        },
        {
          name: "3 tablespoon olive oil",
          image:
            "https://content.instructables.com/F5R/Y0H0/IHVVPRS6/F5RY0H0IHVVPRS6.jpg?auto=webp&frame=1&width=320&md=a3d3b3b73ae688ac87e75fbee681be91",
        },
        {
          name: "2 red bell peppers",
          image:
            "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
        },
        {
          name: "1 habanero pepper you can use just ½",
          image:
            "https://www.livingseedcompany.com/cdn/shop/products/habanero-pepper-capsicum-chinense-694660.png?v=1702656028",
        },
        {
          name: "½ onion",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "1 cup tomato sauce",
          image:
            "https://www.seriouseats.com/thmb/lFQwEdCSP6Y-sVoGqQry-fd0iVw=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/the-best-slow-cooked-italian-american-tomato-sauce-red-sauce-recipe-step-09-05ad7578f9d34a5092086ae959bea8a7.jpg",
        },
        {
          name: "1 teaspoon bouillon powder",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "2 teaspoon black pepper 1 teaspoon to marinate, 1 teaspoon to cook",
          image:
            "https://as1.ftcdn.net/v2/jpg/02/79/24/98/1000_F_279249862_iO0LUG8l16W0vvW3Aw16TYCxLuUwTAk2.jpg",
        },
        {
          name: "1 salt or to taste ½ teaspoon to marinate, ½ to cook",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
        {
          name: "1½ cup water",
          image:
            "https://img-cdn.inc.com/image/upload/w_1024,h_576,c_fill/images/panoramic/getty_1283524975_sgbp6n.jpg",
        },
        {
          name: "oil for frying",
          image:
            "https://content.instructables.com/F5R/Y0H0/IHVVPRS6/F5RY0H0IHVVPRS6.jpg?auto=webp&frame=1&width=320&md=a3d3b3b73ae688ac87e75fbee681be",
        },
      ],
    },
    course: " Main",
    prepTime: "5 minutes",
    cookTime: "20 minutes",
    totalTime: "25 minutes",
    prepLevel: "Easy",
    nutritionalFacts: {
      calories: 206,
      servings: 6,
      fat: "11g",
      protein: "21g",
      carbs: "6g",
      SaturatedFat: "1g",
      PolyunsaturatedFat: "1g",
      MonounsaturatedFat: "5g",
      Cholesterol: "169mg",
      Sodium: "406mg",
      Potassium: "99mg",
      Fiber: "2g",
      Sugar: "3g",
      VitaminA: "1275IU",
      VitaminC: "53mg",
      Calcium: "24mg",
      Iron: "1mg",
    },
    originalSource: "https://lowcarbafrica.com/peppered-croaker-fish/",
  },
  //   {
  //     id: 4,
  //     mealName: "Cabbage Rice",
  //     overview:
  //       "Cabbage rice (also known as riced cabbage) is a quick, easy, and savory side dish and is a great alternative to cauliflower rice with only 7 net carbs! You can easily double it up for a full, satisfying, and nutritious meal.",
  //     image:
  //       "https://thymeandlove.com/wp-content/uploads/2021/09/vegan-fried-rice-720x720.jpg",

  //     recipe: {
  //       instructions: [
  //         "1. Wash the cabbage and pat dry.",
  //         "2. Chop the cabbage and place in the food processor.",
  //         "3. Use the chop function to shred it into smaller pieces.",
  //         "4. Chop the red bell pepper and scallions into small pieces.",
  //         "5. Heat the olive oil in a large skillet, then add the bell pepper and scallions and stir for a minute or two on medium-low heat.",
  //         "6. Add the cabbage, salt, black pepper, and bouillon powder and stir till it begins to soften.",
  //         "7. Stir and leave on medium heat for 5 minutes.",
  //         "8. Serve & enjoy!",
  //       ],
  //       ingredients: [
  //         {
  //           name: "½ cabbage medium",
  //           image: "https://ladgroup.org/wp-content/uploads/2020/12/p5.jpg",
  //         },
  //         {
  //           name: "2 tablespoon olive oil",
  //           image:
  //             "https://content.instructables.com/F5R/Y0H0/IHVVPRS6/F5RY0H0IHVVPRS6.jpg?auto=webp&frame=1&width=320&md=a3d3b3b73ae688ac87e75fbee681be91",
  //         },
  //         {
  //           name: "1 red bell pepper",
  //           image:
  //             "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
  //         },
  //         {
  //           name: "1 bunch scallion",
  //           image:
  //             "https://upload.wikimedia.org/wikipedia/commons/d/da/CSA-Red-Spring-Onions.jpg",
  //         },
  //         {
  //           name: "½ tablespoon bouillon powder",
  //           image:
  //             "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
  //         },
  //         {
  //           name: "½ teaspoon salt to taste",
  //           image:
  //             "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
  //         },
  //         {
  //           name: "1 teaspoon black pepper",
  //           image:
  //             "https://as1.ftcdn.net/v2/jpg/02/79/24/98/1000_F_279249862_iO0LUG8l16W0vvW3Aw16TYCxLuUwTAk2.jpg",
  //         },
  //       ],
  //     },
  //     course: "Snack, Side Dishes and Appetizer",
  //     prepTime: "10 minutes",
  //     cookTime: "10 minutes",
  //     totalTime: "20 minutes",
  //     prepLevel: "Easy",
  //     nutritionalFacts: {
  //       calories: 177,
  //       servings: 3,
  //       fat: "14g",
  //       protein: "3g",
  //       carbs: "12g",
  //       SaturatedFat: "2g",
  //       PolyunsaturatedFat: "2g",
  //       MonounsaturatedFat: "10g",
  //       Cholesterol: "1mg",
  //       Sodium: "634mg",
  //       Potassium: "365mg",
  //       Fiber: "5g",
  //       Sugar: "7g",
  //       VitaminA: "1434IU",
  //       VitaminC: "107mg",
  //       Calcium: "70mg",
  //       Iron: "1mg",
  //     },
  //     originalSource:
  //       "https://lowcarbafrica.com/riced-cabbage-low-carb-side-dish/",
  //   },
  {
    id: 5,
    mealName: "Eggplant Chicken Stew",
    overview:
      "Eggplant chicken stew is a delicious and healthy, nutrient-packed stew that goes perfectly well with low carb rice and noodle dishes. It is low carb and gluten free and has only 6 net carbs!",
    image:
      "https://images.services.kitchenstories.io/5P5n8o8od1MPrNLNHZE0LM_Wu3c=/3840x0/filters:quality(85)/images.kitchenstories.io/wagtailOriginalImages/R2003-photo-final-001.jpg",

    recipe: {
      instructions: [
        "1. Add chicken breast to a pot and pour in enough water to cover it.",
        "2. Add 1 teaspoon of salt and 1 teaspoon of black pepper.",
        "3. Bring to a boil and let it simmer for 10 minutes.",
        "4. Take the chicken out of the broth when done and shred with a fork.",
        "5. Cut the eggplant into large cubes.",
        "6. Boil the eggplants in hot water till the skin is soft and looks very wrinkled.",
        "7. Place in a colander, rinse in water to cool down and peel off the skin.",
        "8. Mash the eggplant till it looks pureed.",
        "9. Leave in a colander to drain a bit more.",
        "10. Blend the tomatoes, bell pepper, habanero pepper, and half an onion together. Chop the other half.",
        "11. Add some palm oil to a pot on medium heat.",
        "12. When it gets hot, add the chopped onions and fry for about a minute.",
        "13. Add the blended tomato and pepper mixture and fry for about 5 minutes.",
        "14. Add your shredded chicken and spices, mix together and cook for 5 more minutes.",
        "15. Add the eggplant and mix thoroughly.",
        "16. Simmer on medium heat for 5 minutes.",
        "17. Serve & enjoy!",
      ],
      ingredients: [
        {
          name: "2 eggplants",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/279/279359/two-eggplants-on-a-wooden-table.jpg",
        },
        {
          name: "4 oz chicken breast",
          image:
            "https://hungryhealthyhappy.com/wp-content/uploads/2023/07/slow-cooker-chicken-breast-featured.jpg",
        },
        {
          name: "½ cup palm oil substitute with olive oil",
          image:
            "https://www.supermart.ng/cdn/shop/files/omkt91106.jpg?v=1692003531",
        },
        {
          name: "2 tomatoes",
          image:
            "https://d2j6dbq0eux0bg.cloudfront.net/images/26774020/1363244751.jpg",
        },
        {
          name: "1 red bell pepper",
          image:
            "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
        },
        {
          name: "1 habanero pepper",
          image:
            "https://www.livingseedcompany.com/cdn/shop/products/habanero-pepper-capsicum-chinense-694660.png?v=1702656028",
        },
        {
          name: "1 teaspoon bouillon powder",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "1 teaspoon cayenne pepper",
          image:
            "https://bonnieplants.com/cdn/shop/products/061422_T118630_202033_Bonnie_HotCayennePepper_ALT_02.jpg?v=1656010754&width=1000",
        },
        {
          name: "2 teaspoon black pepper",
          image:
            "https://as1.ftcdn.net/v2/jpg/02/79/24/98/1000_F_279249862_iO0LUG8l16W0vvW3Aw16TYCxLuUwTAk2.jpg",
        },
        {
          name: "1.5 teaspoon salt to taste",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jp",
        },
      ],
    },
    course: "Main",
    prepTime: "20 minutes",
    cookTime: "15 minutes",
    totalTime: "35 minutes",
    prepLevel: "Med",
    nutritionalFacts: {
      calories: 180,
      servings: 8,
      fat: "14g",
      protein: "3g",
      carbs: "10g",
      SaturatedFat: "1g",
      PolyunsaturatedFat: "1g",
      MonounsaturatedFat: "1g",
      Cholesterol: "5mg",
      Sodium: "82mg",
      Potassium: "403mg",
      Fiber: "4g",
      Sugar: "6g",
      VitaminA: "865IU",
      VitaminC: "28.8mg",
      Calcium: "17mg",
      Iron: "0.5mg",
    },
    originalSource: "https://lowcarbafrica.com/eggplant-chicken-stew/",
  },
  {
    id: 6,
    mealName: "Keto Jollof Cauliflower Rice",
    overview:
      "A low-carb version of the classic Nigerian Jollof Rice, using cauliflower rice for a keto-friendly twist.",
    image:
      "https://divaliciousrecipes.com/wp-content/uploads/2016/08/Cauliflower-Rice.jpg",

    recipe: {
      instructions: [
        "1. Cut the tomatoes, peppers and onions into large chunks.",
        "2. Place in a blender and add half a cup of water.",
        "3. Blend the veggies, pour in a pot and put on the stove on high heat for about 10 minutes.",
        "4. In a separate skillet, pour in palm oil (or olive oil).",
        "5. When heated, add the blended tomato and peppers.",
        "6. Add spices and stir. Leave on medium heat for 10 minutes.",
        "7. Add the cauliflower rice and stir.",
        "8. If using fresh cauliflower rice, add half a cup of water or stock. Omit if using frozen cauliflower rice.",
        "9. Cover the pot for a few minutes.",
        "10. Stir and taste for salt. Adjust accordingly.",
        "11. Serve & enjoy!",
      ],
      ingredients: [
        {
          name: "1 lb cauliflower rice or one head of cauliflower",
          image:
            "https://domf5oio6qrcr.cloudfront.net/medialibrary/5299/h1018g16207257715328.jpg",
        },
        {
          name: "1 tomato",
          image:
            "https://d2j6dbq0eux0bg.cloudfront.net/images/26774020/1363244751.jpg",
        },
        {
          name: "½ habanero pepper or ¼ teaspoon cayenne pepper",
          image:
            "https://www.livingseedcompany.com/cdn/shop/products/habanero-pepper-capsicum-chinense-694660.png?v=170265602",
        },
        {
          name: "1 red bell pepper",
          image:
            "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
        },
        {
          name: "½ onion",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "¼ cup palm oil or olive oil",
          image:
            "https://www.supermart.ng/cdn/shop/files/omkt91106.jpg?v=1692003531",
        },
        {
          name: "½ teaspoon salt",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
        {
          name: "1 teaspoon bouillon powder",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "1 tablespoon ground crayfish optional",
          image:
            "https://housefood.africa/wp-content/uploads/2022/02/crayfish.jpg",
        },
        {
          name: "½ teaspoon black pepper",
          image:
            "https://as1.ftcdn.net/v2/jpg/02/79/24/98/1000_F_279249862_iO0LUG8l16W0vvW3Aw16TYCxLuUwTAk2.jpg",
        },
        {
          name: "½ cup water or stock",
          image:
            "https://img-cdn.inc.com/image/upload/w_1024,h_576,c_fill/images/panoramic/getty_1283524975_sgbp6n.jpg",
        },
      ],
    },
    course: "Main",
    prepTime: "5 minutes",
    cookTime: "15 minutes",
    totalTime: "20 minutes",
    prepLevel: "Easy",
    nutritionalFacts: {
      calories: 70,
      servings: 4,
      fat: "5g",
      protein: "1g",
      carbs: "5g",
      SaturatedFat: "1g",
      PolyunsaturatedFat: "1g",
      MonounsaturatedFat: "1g",
      Cholesterol: "1mg",
      Sodium: "1094mg",
      Potassium: "179mg",
      Fiber: "2g",
      Sugar: "3g",
      VitaminA: "945IU",
      VitaminC: "57mg",
      Calcium: "12mg",
      Iron: "1mg",
    },
    originalSource:
      "https://lowcarbafrica.com/nigerian-cauliflower-jollof-rice/",
  },
  {
    id: 7,
    mealName: "Instant Pot African Lamb Stew",
    overview:
      "If you crave something warm and comforting, this Instant Pot African lamb stew is what you need! Made with tender lamb chunks, veggies, and aromatic spices, this hearty stew will become a family favorite.",
    image:
      "https://lowcarbafrica.com/wp-content/uploads/2021/11/Instant-Pot-African-Lamb-Stew-IG-1.jpg",

    recipe: {
      instructions: [
        "1. Wash and cut the lamb shoulder, if not already cut.",
        "2. Blend the tomatoes, habanero pepper, and red bell pepper in a blender.",
        "3. Chop the onion and set it aside.",
        "4. Turn the Instant Pot to sauté mode.",
        "5. Add some olive oil, then brown the lamb meat for a few minutes.",
        "6. Take out the lamb meat and set it aside.",
        "7. Add the onions and sauté for a few minutes.",
        "8. Add the blended tomato mix and spices. Stir together and scrape the bottom of the pot to deglaze.",
        "9. Add the lamb meat back in the Instant Pot and set it to high pressure for 20 minutes.",
        "10. Let it naturally release the pressure.",
        "11. Turn the Instant Pot to sauté mode and adjust spices.",
        "12. Let the soup simmer till it thickens.",
      ],
      ingredients: [
        {
          name: "1.5 lb lamb meat (lamb shoulder)",
          image:
            "https://www.themeatguy.jp/media/wysiwyg/cookingstudio/recipe/36/36_lamb_00.jpg",
        },
        {
          name: "¼ cup olive oil",
          image:
            "https://content.instructables.com/F5R/Y0H0/IHVVPRS6/F5RY0H0IHVVPRS6.jpg?auto=webp&frame=1&width=320&md=a3d3b3b73ae688ac87e75fbee68",
        },
        {
          name: "3 roma tomatoes",
          image:
            "https://d2j6dbq0eux0bg.cloudfront.net/images/26774020/1363244751.jpg",
        },
        {
          name: "1 red bell pepper",
          image:
            "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
        },
        {
          name: "1 habanero pepper",
          image:
            "https://www.livingseedcompany.com/cdn/shop/products/habanero-pepper-capsicum-chinense-694660.png?v=1702656028",
        },
        [
          {
            name: "½ onion",
            image:
              "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
          },
          {
            name: "½ teaspoon curry",
            image:
              "https://thumbs.dreamstime.com/z/teaspoon-curry-powder-isolated-white-background-261886802.jpg",
          },
          {
            name: "1 teaspoon bouillon powder",
            image:
              "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
          },
          {
            name: "1 teaspoon black pepper",
            image:
              "https://as1.ftcdn.net/v2/jpg/02/79/24/98/1000_F_279249862_iO0LUG8l16W0vvW3Aw16TYCxLuUwTAk2.jpg",
          },
          {
            name: "½ teaspoon salt or to taste",
            image:
              "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
          },
        ],
      ],
    },
    course: "Main ",
    prepTime: "10 minutes",
    cookTime: "40 minutes",
    totalTime: "50 minutes",
    prepLevel: "Med",
    nutritionalFacts: {
      calories: 186,
      servings: 8,
      carbs: "2g",
      protein: "18g",
      fat: "11g",
      SaturatedFat: "3g",
      PolyunsaturatedFat: "1g",
      MonounsaturatedFat: "7g",
      Cholesterol: "55mg",
      Sodium: "268mg",
      Potassium: "334mg",
      Fiber: "1g",
      Sugar: "2g",
      VitaminA: "723IU",
      VitaminC: "24mg",
      Calcium: "12mg",
      Iron: "2mg",
    },
    originalSource: "https://lowcarbafrica.com/instant-pot-african-lamb-stew/",
  },

  {
    id: 8,
    mealName: "Salmon Stew",
    overview:
      "This hearty, easy salmon stew recipe is one of the easiest ways to add more fish to your diet. This dish is also great for home cooks who like to prepare big batches of food and then freeze them for the week.",
    image:
      "https://lowcarbafrica.com/wp-content/uploads/2023/02/Salmon-Stew-IG-1.jpg",

    recipe: {
      instructions: [
        "1. Wash and pat dry the salmon.",
        "2. Sprinkle a little salt on it and rub it all over.",
        "3. Add olive oil to a large saucepan or skillet.",
        "4. When heated, place the salmon in the oil on medium heat.",
        "5. Fry the salmon pieces for about 10 minutes, flipping halfway.",
        "6. Take the salmon out and set it aside.",
        "7. Blend the tomatoes, pepper, and half of the onion. Chop the remaining half onion.",
        "8. Add the chopped onion to the olive oil and sauté till translucent.",
        "9. Pour in the blended pepper mix, and add the spices (except salt).",
        "10. Simmer for 10 minutes.",
        "11. Add the beef broth and salmon and cook for 10 more minutes.",
        "12. Taste for salt and add some if you need (broth usually already contains salt).",
      ],
      ingredients: [
        {
          name: "6 salmon fillets",
          image:
            "https://caudlescatchseafood.com/cdn/shop/products/SalmonOrganicFilletsWatermark.jpg?v=1675453802",
        },
        {
          name: "3 plum tomatoes",
          image:
            "https://d2j6dbq0eux0bg.cloudfront.net/images/26774020/1363244751.jpg",
        },
        {
          name: "1 red bell pepper",
          image:
            "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
        },
        {
          name: "1 habanero pepper or ½",
          image:
            "https://www.livingseedcompany.com/cdn/shop/products/habanero-pepper-capsicum-chinense-694660.png?v=1702656028",
        },
        {
          name: "1 onion",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "½ teaspoon bouillon powder",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "½ teaspoon black pepper",
          image:
            "https://as1.ftcdn.net/v2/jpg/02/79/24/98/1000_F_279249862_iO0LUG8l16W0vvW3Aw16TYCxLuUwTAk2.jpg",
        },
        {
          name: "½ teaspoon curry powder",
          image:
            "https://thumbs.dreamstime.com/z/teaspoon-curry-powder-isolated-white-background-261886802.jpg",
        },
        {
          name: "½ teaspoon thyme",
          image:
            "https://rachelsspicecompany.com/wp-content/uploads/2018/03/ThymeLeaf.jpg",
        },
        {
          name: "½ cup olive oil",
          image:
            "https://content.instructables.com/F5R/Y0H0/IHVVPRS6/F5RY0H0IHVVPRS6.jpg?auto=webp&frame=1&width=320&md=a3d3b3b73ae688ac87e75fbee681be91",
        },
        {
          name: "½ cup broth",
          image:
            "https://media01.stockfood.com/largepreviews/MzQyMTQzNDY2/11036886-2-1-2-Cups-of-Chicken-Broth-in-a-Measuring-Cup.jpg",
        },
        {
          name: "Salt to taste",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
      ],
    },
    course: "Main",
    prepTime: "5 minutes",
    cookTime: "30 minutes",
    totalTime: "35 minutes",
    prepLevel: "Easy",
    nutritionalFacts: {
      calories: 421,
      servings: 6,
      carbs: "4g",
      protein: "34g",
      fat: "29g",
      SaturatedFat: "4g",
      PolyunsaturatedFat: "6g",
      MonounsaturatedFat: "17g",
      Cholesterol: "94mg",
      Sodium: "121mg",
      Potassium: "958mg",
      Fiber: "1g",
      Sugar: "3g",
      VitaminA: "972IU",
      VitaminC: "32mg",
      Calcium: "27mg",
      Iron: "2mg",
    },
    originalSource: "https://lowcarbafrica.com/salmon-stew/",
  },
  {
    id: 9,
    mealName: "Efo Riro - Nigerian Spinach Stew",
    overview:
      "Efo riro (Nigerian Spinach Stew) is a mouthwatering, savory, African stew made with 'Efo Shoko' (Lagos spinach) or 'Efo Tete' (African spinach). Who knew spinach could taste so good?",
    image:
      "https://www.mydiasporakitchen.com/wp-content/uploads/2022/02/12717CC6-10D1-4AEE-9B6A-F110C025BFA8.jpeg",

    recipe: {
      instructions: [
        "1. Blend tomatoes, red bell pepper, habanero pepper, and half of the onion together using a food processor.",
        "2. Chop up the other half of the onion and set aside.",
        "3. If using fresh spinach, chop it up, wash and squeeze, and leave in a colander. If using frozen spinach, microwave for a few minutes.",
        "4. Wash and drain the mushrooms.",
        "5. Heat palm oil in a pot on medium heat, add in your onions and stir for about 2 minutes.",
        "6. Add in your blended tomato and pepper mix and stir for about a minute.",
        "7. Add the bouillon powder, crayfish, black pepper, cayenne pepper, and salt, and cook on medium heat for 5 minutes.",
        "8. If using fresh spinach, add stock or water and bring to a boil.",
        "9. Add the goat meat, mushroom, and any other meat you desire and let boil for another 5 minutes.",
        "10. Add the spinach and stir intermittently for about 5 minutes.",
        "11. Add more salt or spices if desired.",
        "12. Garnish with red pepper flakes, if you want it spicier, and enjoy!",
      ],
      ingredients: [
        {
          name: "8 cups spinach, chopped",
          image:
            "https://hips.hearstapps.com/hmg-prod/images/spinach-1521576635.jpg",
        },
        {
          name: "12 oz cooked meat (goat meat used, but any can be used)",
          image:
            "https://www.excessrawfoods.com/wp-content/uploads/2019/11/goat-meat.jpeg",
        },
        {
          name: "2 cups mushrooms",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/278/278858/mushrooms-in-a-bowel-on-a-dark-table.jpg",
        },
        {
          name: "½ cup palm oil",
          image:
            "https://www.supermart.ng/cdn/shop/files/omkt91106.jpg?v=1692003531",
        },
        {
          name: "2 plum tomatoes (also known as roma tomatoes)",
          image:
            "https://d2j6dbq0eux0bg.cloudfront.net/images/26774020/1363244751.jpg",
        },
        {
          name: "1 red bell pepper",
          image:
            "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
        },
        {
          name: "1 habanero pepper",
          image:
            "https://www.livingseedcompany.com/cdn/shop/products/habanero-pepper-capsicum-chinense-694660.png?v=1702656028",
        },
        {
          name: "1 onion",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "¼ cup water or stock (optional)",
          image:
            "https://img-cdn.inc.com/image/upload/w_1024,h_576,c_fill/images/panoramic/getty_1283524975_sgbp6n.jpg",
        },
        {
          name: "2 teaspoons bouillon powder",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "2 teaspoons cayenne pepper",
          image:
            "https://bonnieplants.com/cdn/shop/products/061422_T118630_202033_Bonnie_HotCayennePepper_ALT_02.jpg?v=1656010754&width=1000",
        },
        {
          name: "1 teaspoon black pepper",
          image:
            "https://as1.ftcdn.net/v2/jpg/02/79/24/98/1000_F_279249862_iO0LUG8l16W0vvW3Aw16TYCxLuUwTAk2.jpg",
        },
        {
          name: "2 teaspoons ground crayfish",
          image:
            "https://housefood.africa/wp-content/uploads/2022/02/crayfish.jpg",
        },
        {
          name: "1 teaspoon pepper flakes (optional)",
          image:
            "https://oldworldgardenfarms.com/wp-content/uploads/2018/09/featured-red-pepper-flakes.jpg",
        },
        {
          name: "1 teaspoon salt or to taste",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
      ],
    },
    course: "Main",
    prepTime: "10 minutes",
    cookTime: "20 minutes",
    totalTime: "30 minutes",
    prepLevel: "Med",
    nutritionalFacts: {
      calories: 246,
      servings: 8,
      carbs: "13g",
      protein: "16g",
      fat: "16g",
      Sodium: "598mg",
      Potassium: "209mg",
      Fiber: "8g",
      Sugar: "3g",
      VitaminA: "23820IU",
      VitaminC: "32.7mg",
      Calcium: "226mg",
      Iron: "2.2mg",
    },
    originalSource: "https://lowcarbafrica.com/efo-riro-nigerian-spinach-stew/",
  },
  {
    id: 10,
    mealName: "Egg Stew (Nigerian Egg Sauce)",
    overview:
      "This egg stew recipe is one of the easiest and quickest Nigerian stews you can make. It tastes incredible and will leave you begging for more!",
    image: "https://eatwellabi.com/wp-content/uploads/2022/11/egg-sauce-37.jpg",

    recipe: {
      instructions: [
        "1. Coarsely blend the tomatoes, red bell pepper, habanero pepper, and half of the onion.",
        "2. Chop the other half onion and mince the garlic.",
        "3. Break and whisk the eggs.",
        "4. Saute the onions and garlic in olive oil till fragrant.",
        "5. Pour in the blended tomatoes and pepper mix and stir.",
        "6. Let simmer for 5 mins on medium-high heat or until most of the liquid has evaporated.",
        "7. Add the curry, thyme, and bouillon and let it cook for 2 more minutes.",
        "8. Pour in the eggs, wait 2 mins, then stir.",
        "9. Stir the eggs till they are well mixed in.",
        "10. Taste for salt and add if you need some.",
        "11. Garnish with your herb of choice and enjoy!",
      ],
      ingredients: [
        {
          name: "6 eggs, large",
          image:
            "https://cdn.britannica.com/94/151894-050-F72A5317/Brown-eggs.jpg",
        },
        {
          name: "2 tomatoes",
          image:
            "https://d2j6dbq0eux0bg.cloudfront.net/images/26774020/1363244751.jpg",
        },
        {
          name: "½ onion",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "2 garlic cloves",
          image:
            "https://lilluna.com/wp-content/uploads/2020/01/minced-garlic-resize-5.jpg",
        },
        {
          name: "½ cup olive oil",
          image:
            "https://content.instructables.com/F5R/Y0H0/IHVVPRS6/F5RY0H0IHVVPRS6.jpg?auto=webp&frame=1&width=320&md=a3d3b3b73ae688ac87e75fbee681be91",
        },
        {
          name: "1 red bell pepper",
          image:
            "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
        },
        {
          name: "1 habanero pepper",
          image:
            "https://www.livingseedcompany.com/cdn/shop/products/habanero-pepper-capsicum-chinense-694660.png?v=1702656028",
        },
        {
          name: "1 teaspoon curry",
          image:
            "https://thumbs.dreamstime.com/z/teaspoon-curry-powder-isolated-white-background-261886802.jpg",
        },
        {
          name: "½ teaspoon thyme",
          image:
            "https://rachelsspicecompany.com/wp-content/uploads/2018/03/ThymeLeaf.jpg",
        },
        {
          name: "1 tablespoon bouillon",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "Salt to taste",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
      ],
    },
    course: "Soups & Stews",
    prepTime: "5 minutes",
    cookTime: "15 minutes",
    totalTime: "20 minutes",
    prepLevel: "Easy",
    nutritionalFacts: {
      calories: 358,
      servings: 4,
      carbs: "6g",
      protein: "9g",
      fat: "34g",
      SaturatedFat: "6g",
      Cholesterol: "246mg",
      Sodium: "113mg",
      Potassium: "300mg",
      Fiber: "1g",
      Sugar: "3g",
      VitaminA: "2021IU",
      VitaminC: "50mg",
      Calcium: "46mg",
      Iron: "2mg",
    },
    originalSource: "https://lowcarbafrica.com/egg-stew-nigerian-egg-sauce/",
  },
  {
    id: 11,
    mealName: "Okro Soup (West African Okra Soup)",
    overview:
      "Okro soup (African okra soup) is a delicious soup commonly eaten in Nigeria and other West African countries and served with fufu dishes.",
    image:
      "https://nigerianfoodiehub.com/wp-content/uploads/2022/02/Attachment-1-30-1.jpg",

    recipe: {
      instructions: [
        "1. Wash the goat meat and place in a large pot.",
        "2. Chop half an onion and add salt, black pepper, bouillon powder.",
        "3. Boil for about 30 minutes, stirring intermittently. The meat should be quite tender but not too soft when done.",
        "4. If applicable, soak the smoked catfish and locust bean, separately in hot water for about 5 minutes.",
        "5. Break apart the smoked catfish and remove the bones.",
        "6. Wash and drain the locust bean.",
        "7. Trim the ends off the okra, wash, and pat dry.",
        "8. Chop okra using a food processor and set aside.",
        "9. Pour palm oil in a large pan on medium heat.",
        "10. Add onions and stir fry for 2 minutes till it turns just a bit brown.",
        "11. Add okra to palm oil and stir for about 5 minutes.",
        "12. Pour in the meat stock from boiling the meat. You can add water if you don’t have any but might need to increase the spices added.",
        "13. Add the meat, shrimp, bell pepper, smoked catfish, and locust bean and bring to a boil. You can add or subtract anything on this list, just make it your own!",
        "14. Mix in the spices - black pepper, ground pepper, ground crayfish, bouillon powder, and salt.",
        "15. Let it simmer for about 5 minutes if you’re okay with a coarser texture or a few minutes longer if you want a finer texture.",
        "16. Add more salt or spices if you desire a stronger flavor.",
        "17. Serve and enjoy with low carb fufu!",
      ],
      ingredients: [
        {
          name: "1 lb goat meat",
          image:
            "https://www.excessrawfoods.com/wp-content/uploads/2019/11/goat-meat.jpeg",
        },
        {
          name: "1 tablespoon bouillon powder",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "1 teaspoon black pepper",
          image:
            "https://as1.ftcdn.net/v2/jpg/02/79/24/98/1000_F_279249862_iO0LUG8l16W0vvW3Aw16TYCxLuUwTAk2.jpg",
        },
        {
          name: "½ onion",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "1 teaspoon salt",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
        {
          name: "25 okra pieces (4 cups chopped)",
          image:
            "https://4.bp.blogspot.com/-b4v2MRt6IO4/UeEN3mCKhlI/AAAAAAAABJ4/eJcePJUHcpc/s1600/okra-cedf06a6a383adb0af179cabd2cee13df5fa76f7-s6-c30.jpg",
        },
        {
          name: "1.5 cups shrimp",
          image:
            "https://cookanyday.com/cdn/shop/products/ANYDAY_Q1_20239463_grande.jpg?v=1672945401",
        },
        {
          name: "1 smoked catfish",
          image:
            "https://freshtodommot.com/cdn/shop/products/eja-kika-smoked-panla.jpg?v=1591946606",
        },
        {
          name: "½ cup palm oil",
          image:
            "https://www.supermart.ng/cdn/shop/files/omkt91106.jpg?v=1692003531",
        },
        {
          name: "½ onion",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "2 tablespoon Iru locust bean (optional)",
          image:
            "Ihttps://www.capecrystalbrands.com/cdn/shop/articles/unlocking-the-health-benefits-of-locust-bean-gum-107218.png?v=1695782336",
        },
        {
          name: "2 mini bell peppers",
          image:
            "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
        },
        {
          name: "1 tablespoon bouillon powder",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "1 teaspoon ground crayfish",
          image:
            "https://housefood.africa/wp-content/uploads/2022/02/crayfish.jpg",
        },
        {
          name: "1 teaspoon cayenne pepper or dry ground pepper",
          image:
            "https://bonnieplants.com/cdn/shop/products/061422_T118630_202033_Bonnie_HotCayennePepper_ALT_02.jpg?v=1656010754&width=1000",
        },
        {
          name: "Meat stock (can substitute with water)",
          image:
            "https://media01.stockfood.com/largepreviews/MzQyMTQzNDY2/11036886-2-1-2-Cups-of-Chicken-Broth-in-a-Measuring-Cup.jpg",
        },
        {
          name: "1 teaspoon salt",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
      ],
    },
    course: "Main",
    prepTime: "30 minutes",
    cookTime: "15 minutes",
    totalTime: "45 minutes",
    prepLevel: "Med",
    nutritionalFacts: {
      calories: 236,
      servings: 12,
      carbs: "3g",
      protein: "26g",
      fat: "12g",
      Cholesterol: "83mg",
      Sodium: "763mg",
      Potassium: "404mg",
      Fiber: "1g",
      Sugar: "1g",
      VitaminA: "185IU",
      VitaminC: "7.8mg",
      Calcium: "36mg",
      Iron: "0.6mg",
    },
    originalSource: "https://lowcarbafrica.com/okro-soup-african-okra-soup/",
  },
  {
    id: 12,
    mealName: "Egusi Soup - Nigerian Melon Seed Stew",
    overview:
      "Egusi Soup (also known as Egusi Stew) is a rich and savory West African soup made with ground melon seeds and eaten with fufu dishes. It is an amazing soup the whole family will enjoy!",
    image:
      "https://nigerianfoodiehub.com/wp-content/uploads/2023/08/IMG_1965.jpg",

    recipe: {
      instructions: [
        "1. If using smoked catfish, soak in hot water for 10 minutes.",
        "2. Break the dried catfish into smaller pieces and remove as many pieces of bones as you can.",
        "3. Chop the onions and spinach.",
        "4. Blend the tomatoes, red bell pepper and habanero pepper. Add as little water as possible.",
        "5. Heat the palm oil on medium heat for a few minutes, then add chopped onions.",
        "6. After it becomes translucent, add your blended tomato and pepper mix and stir.",
        "7. Add your smoked fish and cook on medium heat for about 10 minutes.",
        "8. Add broth or water and bring to a boil.",
        "9. Pour the ground egusi seeds into a bowl, add a sprinkling of water and form them into balls.",
        "10. Add the egusi seeds to the soup and stir.",
        "11. Bring to a boil and let it cook for 5 more minutes.",
        "12. Add the goat meat, cow feet, crayfish, bouillon, and cayenne pepper.",
        "13. Taste for salt and add some if you need to.",
        "14. Let it boil for 10 minutes on medium-high heat.",
        "15. Add spinach and leave on low heat for 5 minutes.",
        "16. Serve with your choice of fufu and enjoy!",
      ],
      ingredients: [
        {
          name: "2 cups egusi seeds melon seeds",
          image:
            "https://somiigbene.com/wp-content/uploads/2021/03/EGUSI-SEEDS.jpg",
        },
        {
          name: "400 g goat meat cooked",
          image:
            "https://www.excessrawfoods.com/wp-content/uploads/2019/11/goat-meat.jpeg",
        },
        {
          name: "200 g cow feet cooked",
          image:
            "https://recipes.net/wp-content/uploads/2023/11/how-to-cook-jamaican-cow-foot-1699448376.jpg",
        },
        {
          name: "½ cup palm oil",
          image: "IMAGE_URL_OF_PALM_OIL",
        },
        {
          name: "2 cups spinach",
          image:
            "https://hips.hearstapps.com/hmg-prod/images/spinach-1521576635.jpg",
        },
        {
          name: "1 smoked catfish optional",
          image:
            "https://www-konga-com-res.cloudinary.com/w_auto,f_auto,fl_lossy,dpr_auto,q_auto/media/catalog/product/O/R/125317_1593817524.jpg",
        },
        {
          name: "1 tomato",
          image:
            "https://d2j6dbq0eux0bg.cloudfront.net/images/26774020/1363244751.jpg",
        },
        {
          name: "1 red bell pepper",
          image:
            "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
        },
        {
          name: "1 habanero pepper",
          image:
            "https://www.livingseedcompany.com/cdn/shop/products/habanero-pepper-capsicum-chinense-694660.png?v=1702656028",
        },
        {
          name: "1 onion",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "½ tablespoon cayenne pepper",
          image:
            "https://bonnieplants.com/cdn/shop/products/061422_T118630_202033_Bonnie_HotCayennePepper_ALT_02.jpg?v=1656010754&width=1000",
        },
        {
          name: "1 tablespoon bouillon powder or maggi cubes",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "2 tablespoon crayfish",
          image:
            "https://housefood.africa/wp-content/uploads/2022/02/crayfish.jpg",
        },
        {
          name: "1 tablespoon salt to boil the meat",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
        {
          name: "1 tablespoon black pepper to boil the meat",
          image:
            "https://as1.ftcdn.net/v2/jpg/02/79/24/98/1000_F_279249862_iO0LUG8l16W0vvW3Aw16TYCxLuUwTAk2.jpg",
        },
      ],
    },
    course: "Main",
    prepTime: "10 minutes",
    cookTime: "35 minutes",
    totalTime: "45 minutes",
    prepLevel: "Med",
    nutritionalFacts: {
      calories: 397,
      servings: 8,
      carbs: "5g",
      protein: "32g",
      fat: "27g",
      Cholesterol: "70mg",
      Sodium: "59mg",
      Potassium: "511mg",
      Fiber: "1g",
      Sugar: "1g",
      VitaminA: "1375IU",
      VitaminC: "25mg",
      Calcium: "28mg",
      Iron: "0.7mg",
    },
    originalSource: "https://lowcarbafrica.com/nigerian-egusi-soup/",
  },
  {
    id: 13,
    mealName: "Banga Soup (Palm Nut Soup)",
    overview:
      "Banga Soup, also known as Palm Nut Soup or Ofe Akwu, is one of my favorite Nigerian soups and has a pleasant, nutty flavor. This delightful soup will leave you licking your fingers!",
    image:
      "https://www.myactivekitchen.com/wp-content/uploads/2015/03/niger-delta-banga-soup-recipe-img-7.jpg",

    recipe: {
      instructions: [
        "1. Cut and clean your catfish if you bought it whole.",
        "2. Pour the palm nut concentrate into a pot.",
        "3. Add in the shrimps and spices and let it boil for 10 minutes.",
        "4. Add your catfish and simmer for about 15 minutes.",
        "5. When the oil from the palm nut starts to rise to the surface, your banga soup is ready.",
        "6. Make the palm nut concentrate using a mortar and pestle (optional)",
        "7. Wash the palm nut seeds, and place them in a pot with enough water to cover them.",
        "8. Boil till tender (about 20 minutes).",
        "9. Pour the palm kernel seeds into a mortar and use a pestle to gently pound and mash them.",
        "10. When the palm nut seeds look mushy, transfer them to another large bowl. Add some water, squeeze the palm nuts, and drain into a separate bowl.",
        "11. Repeat several times till the water looks a bit lighter.",
        "12. Strain the water from washing the palm nut into a pot.",
        "13. Boil for about 30 minutes or an hour until the liquid has a thicker viscosity.",
        "14. Continue cooking the banga soup using instructions from above.",
      ],
      ingredients: [
        {
          name: "1 can Palm-nut concentrate",
          image:
            "Ihttps://fiduciaafricanshop.com/wp-content/uploads/2019/03/71G5Vcva2DL._SX425_.jpg",
        },
        {
          name: "2 lbs catfish",
          image:
            "https://africanunique.com/cdn/shop/products/smokedcatfish_530x@2x.jpg?v=1609982165",
        },
        {
          name: "8 oz shrimps",
          image:
            "https://cookanyday.com/cdn/shop/products/ANYDAY_Q1_20239463_grande.jpg?v=1672945401S",
        },
        {
          name: "1 tablespoon bouillon powder",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "1 teaspoon cayenne pepper",
          image:
            "https://bonnieplants.com/cdn/shop/products/061422_T118630_202033_Bonnie_HotCayennePepper_ALT_02.jpg?v=1656010754&width=1000",
        },
        {
          name: "1 tablespoon ground crayfish",
          image:
            "https://housefood.africa/wp-content/uploads/2022/02/crayfish.jpg",
        },
        {
          name: "1.5 tablespoon banga spice",
          image: "https://www.africanfoods.co.uk/images/Banga_Spice_Mix_2.jpg",
        },
        {
          name: "1 teaspoon salt or to taste",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
      ],
    },
    course: "Soups & Stews",
    prepTime: "5 minutes",
    cookTime: "30 minutes",
    totalTime: "35 minutes",
    prepLevel: "Easy",
    nutritionalFacts: {
      calories: 340,
      servings: 8,
      carbs: "4g",
      protein: "25g",
      fat: "25g",
      SaturatedFat: "1g",
      Cholesterol: "137mg",
      Sodium: "722mg",
      Potassium: "429mg",
      Fiber: "3g",
      Sugar: "1g",
      VitaminA: "57IU",
      VitaminC: "2mg",
      Calcium: "57mg",
      Iron: "1mg",
    },
    originalSource: "https://lowcarbafrica.com/banga-soup-palm-nut-soup/",
  },
  {
    id: 14,
    mealName: "Ogbono Soup (Draw Soup)",
    overview:
      "Ogbono soup is a rich, Nigerian soup made from Ogbono seeds (ground African bush mango seeds), palm oil and assorted meats. Eat ogbono soup with fufu dishes for a delectable Nigerian dish!",
    image: "https://www.battabox.com/ajako/2023/01/ogbono-soup-served.webp",

    recipe: {
      instructions: [
        "1. Cook all the meat you’re going to use, with some salt, pepper, onion, and bouillon powder until it becomes tender.",
        "2. Chop the spinach and set aside.",
        "3. If using whole ogbono seeds, grind it using a mortar and pestle set or a coffee grinder. Skip if using pre-ground seeds.",
        "4. When the assorted meats are ready, drain the stock out but save some of the stock so you can use it to flavor the ogbono soup.",
        "5. In another pot, add some palm oil on low heat and add the ogbono seeds to it.",
        "6. Mix in the ogbono seeds and break any lumps with a serving spoon. Stir until it’s thoroughly coated.",
        "7. Add the stock from the cooked meats and stir.",
        "8. Add water and stir until you reach your desired thickness.",
        "9. Bring to a boil, then set to low heat and let it cook for about 10 minutes, stirring intermittently.",
        "10. Add in the meats, bouillon powder, cayenne pepper, crayfish.",
        "11. Taste for salt and add some if needed.",
        "12. If it becomes too thick, add a little more water.",
        "13. Let simmer for about 10 minutes.",
        "14. Add the chopped spinach and let simmer on low for about 2 more minutes.",
        "15. Your ogbono soup is ready to serve!",
      ],
      ingredients: [
        {
          name: "1 cup ogbono seeds",
          image: "https://afrominimart.com/wp-content/uploads/2021/03/ognb.jpg",
        },
        {
          name: "1 lb beef",
          image:
            "https://www.eatingwellorganics.ca/cdn/shop/products/Beef-Stew-Beef.jpg?v=1623765042",
        },
        {
          name: "8 oz shaki (tripe)",
          image:
            "https://biscuitboneblog.files.wordpress.com/2015/05/img_6151.jpg",
        },
        {
          name: "12 oz cow feet",
          image:
            "https://atasteozimboivyskitchenette.files.wordpress.com/2016/10/img_9941.jpg",
        },
        {
          name: "¾ cup palm oil",
          image:
            "https://www.supermart.ng/cdn/shop/files/omkt91106.jpg?v=1692003531",
        },
        {
          name: "1 cup chopped spinach or your choice of leafy greens",
          image:
            "https://hips.hearstapps.com/hmg-prod/images/spinach-1521576635.jpg",
        },
        {
          name: "1 cup onions chopped",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "1 tablespoon bouillon powder or 2 maggi stock cubes",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "1 tablespoon ground crayfish",
          image:
            "https://housefood.africa/wp-content/uploads/2022/02/crayfish.jpg",
        },
        {
          name: "1 tablespoon cayenne pepper",
          image:
            "https://bonnieplants.com/cdn/shop/products/061422_T118630_202033_Bonnie_HotCayennePepper_ALT_02.jpg?v=1656010754&width=1000",
        },
        {
          name: "2 cups meat stock",
          image:
            "https://media01.stockfood.com/largepreviews/MzQyMTQzNDY2/11036886-2-1-2-Cups-of-Chicken-Broth-in-a-Measuring-Cup.jpg",
        },
        {
          name: "2 cups water",
          image:
            "https://img-cdn.inc.com/image/upload/w_1024,h_576,c_fill/images/panoramic/getty_1283524975_sgbp6n.jpg",
        },
        {
          name: "salt to taste",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
      ],
    },
    course: "Main",
    prepTime: "5 minutes",
    cookTime: "40 minutes",
    totalTime: "45 minutes",
    prepLevel: "Easy",
    nutritionalFacts: {
      calories: 425,
      servings: 8,
      carbs: "6g",
      protein: "22g",
      fat: "20g",
      SaturatedFat: "19g",
      Cholesterol: "40mg",
      Sodium: "228mg",
      Potassium: "195mg",
      Fiber: "1g",
      Sugar: "1g",
      VitaminA: "317IU",
      VitaminC: "3mg",
      Calcium: "19mg",
      Iron: "2mg",
    },
    originalSource: "https://lowcarbafrica.com/ogbono-soup-draw-soup/",
  },
  {
    id: 15,
    mealName: "Chicken Pepper Soup",
    overview:
      "This chicken pepper soup is another well-sought-after delicacy in many Nigerian homes. This hearty soup can be enjoyed any time, no matter the season!",
    image:
      "https://lowcarbafrica.com/wp-content/uploads/2021/07/Chicken-Pepper-Soup-IG-1.jpg",

    recipe: {
      instructions: [
        "1. Rinse the chicken breast and drain.",
        "2. Cut chicken breast into small cubes.",
        "3. Add the chicken to a pot, and then add 4 cups of water.",
        "4. Add the bouillon powder and smoked paprika.",
        "5. Bring to a boil and simmer for 10 minutes.",
        "6. Add the pepper soup spice and habanero pepper.",
        "7. Taste for salt before adding any.",
        "8. Simmer for 10 more minutes or until chicken is tender.",
      ],
      ingredients: [
        {
          name: "2 lbs chicken breast",
          image:
            "https://hungryhealthyhappy.com/wp-content/uploads/2023/07/slow-cooker-chicken-breast-featured.",
        },
        {
          name: "1 habanero pepper",
          image:
            "https://www.livingseedcompany.com/cdn/shop/products/habanero-pepper-capsicum-chinense-694660.png?v=1702656028",
        },
        {
          name: "1 tablespoon bouillon powder",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "1 tablespoon pepper soup spice",
          image:
            "https://lowcarbafrica.com/wp-content/uploads/2020/03/Pepper-Soup-Spice-labeled.jpg",
        },
        {
          name: "½ teaspoon smoked paprika",
          image:
            "https://i0.wp.com/nextcashandcarry.com.ng/wp-content/uploads/2022/07/spspot2873.png?fit=512%2C512&ssl=1",
        },
        {
          name: "½ teaspoon salt or to taste",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
        {
          name: "3-4 cups water",
          image:
            "https://img-cdn.inc.com/image/upload/w_1024,h_576,c_fill/images/panoramic/getty_1283524975_sgbp6n.jpg",
        },
        {
          name: "basil leaf (optional) to garnish",
          image: "https://cdn.britannica.com/87/207887-050-F48CB55D/basil.jpg",
        },
      ],
    },
    course: "Soups & Stews",
    prepTime: "5 minutes",
    cookTime: "20 minutes",
    totalTime: "25 minutes",
    prepLevel: "Easy",
    nutritionalFacts: {
      calories: 262,
      servings: 4,
      carbs: "1g",
      protein: "48g",
      fat: "6g",
      SaturatedFat: "1g",
      PolyunsaturatedFat: "1g",
      MonounsaturatedFat: "2g",
      TransFat: "1g",
      Cholesterol: "145mg",
      Sodium: "597mg",
      Potassium: "853mg",
      Fiber: "1g",
      Sugar: "1g",
      VitaminA: "92IU",
      VitaminC: "6mg",
      Calcium: "18mg",
      Iron: "1mg",
    },
    originalSource: "https://lowcarbafrica.com/chicken-pepper-soup/",
  },
  {
    id: 16,
    mealName: "Fish Pepper Soup",
    overview:
      "Fish pepper soup is a soothing West African Pepper Soup, made with just a few ingredients - catfish, vegetables, and spices.",
    image:
      "https://lagosbukagrillandbar.com/wp-content/uploads/2021/07/Catfish-Peppersoup.jpeg",

    recipe: {
      instructions: [
        "1. Boil some water.",
        "2. Gut and clean out the inner parts of the catfish.",
        "3. Clean the catfish using lime or lemon to scrub the skin and insides as well.",
        "4. Cut the fish into medium-sized pieces.",
        "5. Chop habanero pepper into tiny pieces.",
        "6. Loosely chop the basil leaves and set aside.",
        "7. Pour the hot water over the fish and leave for about 30 seconds.",
        "8. Drain out the hot water, rinse under the tap and place in a pot.",
        "9. Add about 6 cups of water or until the fish is completely submerged.",
        "10. Add the bouillon powder and bring to a boil.",
        "11. Add the smoked paprika, ground crayfish, salt, and chopped habanero pepper.",
        "12. Only add the cayenne pepper if you want it spicier.",
        "13. Let simmer for 20 minutes on medium heat.",
        "14. Taste and adjust the salt if needed.",
        "15. Add the chopped basil leaves and simmer for 5 more minutes.",
        "16. Serve and enjoy!",
      ],
      ingredients: [
        {
          name: "2 lbs catfish",
          image:
            "https://kitchenatics.com/wp-content/uploads/2020/06/Fish-fillet-1.jpg",
        },
        {
          name: "1 habanero pepper (chopped)",
          image:
            "https://www.livingseedcompany.com/cdn/shop/products/habanero-pepper-capsicum-chinense-694660.png?v=1702656028",
        },
        {
          name: "1 teaspoon smoked paprika",
          image:
            "https://i0.wp.com/nextcashandcarry.com.ng/wp-content/uploads/2022/07/spspot2873.png?fit=512%2C512&ssl=1",
        },
        {
          name: "1 tablespoon bouillon powder",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jp",
        },
        {
          name: "1 tablespoon pepper soup spice",
          image:
            "https://www.gomarket.com.ng/wp-content/uploads/2020/05/Spice-Supreme-pepper-soup.jpg",
        },
        {
          name: "1 teaspoon crayfish (substitute with 1 tablespoon coconut aminos)",
          image:
            "https://housefood.africa/wp-content/uploads/2022/02/crayfish.jpg",
        },
        {
          name: "½ cup basil leaf (loosely chopped)",
          image:
            "https://www.wikihow.com/images/thumb/9/91/Chop-Basil-Step-9.jpg/550px-nowatermark-Chop-Basil-Step-9.jpg",
        },
        {
          name: "1 lime/lemon (to clean the fish)",
          image:
            "https://upload.wikimedia.org/wikipedia/commons/thumb/f/f7/Lemon_-_whole_and_split.jpg/405px-Lemon_-_whole_and_split.jpg",
        },
        {
          name: "2 tsp salt (or to taste)",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
        {
          name: "½ teaspoon cayenne pepper (optional)",
          image:
            "https://bonnieplants.com/cdn/shop/products/061422_T118630_202033_Bonnie_HotCayennePepper_ALT_02.jpg?v=1656010754&width=1000",
        },
        {
          name: "6 cups water (or till the fish is completely covered)",
          image:
            "https://img-cdn.inc.com/image/upload/w_1024,h_576,c_fill/images/panoramic/getty_1283524975_sgbp6n.jpg",
        },
      ],
    },
    course: "Soups & Stews",
    prepTime: "10 minutes",
    cookTime: "30 minutes",
    totalTime: "40 minutes",
    prepLevel: "Easy",
    nutritionalFacts: {
      calories: 148,
      servings: 6,
      carbs: "1g",
      protein: "25g",
      fat: "4g",
      SaturatedFat: "1g",
      Cholesterol: "89mg",
      Sodium: "1071mg",
      Potassium: "549mg",
      Fiber: "1g",
      Sugar: "1g",
      VitaminA: "325IU",
      VitaminC: "3mg",
      Calcium: "29mg",
      Iron: "1mg",
    },
    originalSource: "https://lowcarbafrica.com/fish-pepper-soup/",
  },
  {
    id: 17,
    mealName: "Edikang Ikong Soup (Nigerian Vegetable Soup)",
    overview:
      "Edikang Ikong soup is a traditional Nigerian soup that is highly nutritious and protein-packed. This finger-licking delicacy is also commonly known as Nigerian Vegetable Soup.",
    image:
      "https://mandifoods.com.ng/wp-content/uploads/2021/01/Vegetable-Soup.jpg",

    recipe: {
      instructions: [
        "1. Prepare the goat meat:",
        "   - Wash and cut the goat meat (if not already cut).",
        "   - Add to a pot and pour enough water to cover it.",
        "   - Add the chopped onions, bouillon/maggi, salt, and pepper.",
        "   - Boil till soft (about 40-45 minutes).",
        "   - Take out the meat and leave the stock in the pot for the soup.",
        "2. Prepare the leaves:",
        "   - While the goat meat is cooking, wash and drain the leaves.",
        "   - Leave in a colander while you prepare the soup.",
        "3. Prepare the soup:",
        "   - Add some water to the stock in the pot.",
        "   - Add the shrimp, smoked catfish, beef & spices.",
        "   - Boil for 15 - 20 minutes.",
        "   - Add the waterleaf and ugu leaves.",
        "   - Stir and taste for salt.",
        "   - Pour in the palm oil.",
        "   - Simmer for 10 mins on low heat.",
      ],
      ingredients: [
        {
          name: "2 lbs goat meat",
          image:
            "https://www.excessrawfoods.com/wp-content/uploads/2019/11/goat-meat.jpeg",
        },
        {
          name: "1 teaspoon bouillon powder (or 2 maggi cubes)",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "1 teaspoon black pepper",
          image:
            "https://as1.ftcdn.net/v2/jpg/02/79/24/98/1000_F_279249862_iO0LUG8l16W0vvW3Aw16TYCxLuUwTAk2.jpg",
        },
        {
          name: "1 teaspoon salt (or to taste)",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
        {
          name: "½ onion",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "Enough water to cover the meat",
          image:
            "https://img-cdn.inc.com/image/upload/w_1024,h_576,c_fill/images/panoramic/getty_1283524975_sgbp6n.jpg",
        },
        {
          name: "2 cups waterleaf (malabar spinach)",
          image:
            "https://guardian.ng/wp-content/uploads/life/2016/03/waterleaf.jpeg",
        },
        {
          name: "4 cups ugwu leaves (fluted pumpkin leaves)",
          image:
            "https://freshtodommot.com/cdn/shop/products/ugwu.jpg?v=1585827248",
        },
        {
          name: "¾ cup palm oil",
          image:
            "https://www.supermart.ng/cdn/shop/files/omkt91106.jpg?v=1692003531",
        },
        {
          name: "2 cups stock",
          image:
            "https://media01.stockfood.com/largepreviews/MzQyMTQzNDY2/11036886-2-1-2-Cups-of-Chicken-Broth-in-a-Measuring-Cup.jpg",
        },
        {
          name: "2 cups water",
          image:
            "https://img-cdn.inc.com/image/upload/w_1024,h_576,c_fill/images/panoramic/getty_1283524975_sgbp6n.jpg",
        },
        {
          name: "6 oz smoked/dried fish",
          image:
            "https://africanunique.com/cdn/shop/products/smokedcatfish_530x@2x.jpg?v=1609982165",
        },
        {
          name: "8 oz shrimp",
          image:
            "https://cookanyday.com/cdn/shop/products/ANYDAY_Q1_20239463_grande.jpg?v=1672945401",
        },
        {
          name: "2 teaspoon ground crayfish",
          image:
            " https://housefood.africa/wp-content/uploads/2022/02/crayfish.jpg",
        },
        {
          name: "1 teaspoon cayenne pepper",
          image:
            "https://bonnieplants.com/cdn/shop/products/061422_T118630_202033_Bonnie_HotCayennePepper_ALT_02.jpg?v=1656010754&width=1000",
        },
        {
          name: "1 teaspoon bouillon powder (or maggi cubes)",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "1 teaspoon salt (or to taste)",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
      ],
    },
    course: "Main",
    prepTime: "45 minutes",
    cookTime: "30 minutes",
    totalTime: "1 hour 15 minutes",
    prepLevel: "Med",
    nutritionalFacts: {
      calories: 193,
      servings: 8,
      carbs: "2g",
      protein: "36g",
      fat: "4g",
      SaturatedFat: "1g",
      Cholesterol: "71mg",
      Sodium: "644mg",
      Potassium: "28mg",
      Fiber: "1g",
      Sugar: "1g",
      VitaminA: "697IU",
      VitaminC: "2mg",
      Calcium: "73mg",
      Iron: "1mg",
    },
    originalSource:
      "https://lowcarbafrica.com/edika-ikong-soup-nigerian-vegetable-soup/",
  },
  {
    id: 18,
    mealName: "Bitter Leaf Soup",
    overview:
      "Bitter leaf soup is a popular, traditional soup enjoyed by different tribes in Nigeria. It can be eaten with various fufu dishes and is so healthy and satisfying!",
    image: "https://niammy.com/img/public/990/201903/1553547369_24159.jpg",

    recipe: {
      instructions: [
        "1. Chop the onions and blend the tomatoes, red bell pepper, and habanero pepper with very little water.",
        "2. If using frozen bitter leaf, let it thaw out for an hour or microwave to thaw it out quickly. Rinse under running water and drain.",
        "3. If using fresh bitter leaf, wash thoroughly in plenty of water, with a little salt, till it runs clear.",
        "4. Heat some palm oil in a pot and add some onions.",
        "5. Let the onions sauté for a minute or two.",
        "6. Pour in the blended tomatoes and cook for 5 mins.",
        "7. Add your meat, smoked fish, spices, and beef stock, and simmer for a few minutes.",
        "8. Then, add the ground egusi seeds and stir.",
        "9. Bring to a boil and simmer for 10 mins.",
        "10. Add bitter leaf and stir together.",
        "11. Cook on medium-low heat till the bitter leaf is wilted.",
        "12. Serve and enjoy!",
      ],
      ingredients: [
        {
          name: "4 cups bitter leaf (about 450g)",
          image:
            "https://motherlandharvest.com/cdn/shop/products/F0C6C0D5-2D76-441A-AFFF-7E927B7361FA.jpg?v=1674152003",
        },
        {
          name: "1 cup ground egusi seeds",
          image:
            "https://upload.wikimedia.org/wikipedia/commons/d/d0/EGUSI.JPG",
        },
        {
          name: "1 lb beef (already cooked)",
          image:
            "https://www.eatingwellorganics.ca/cdn/shop/products/Beef-Stew-Beef.jpg?v=1623765042",
        },
        {
          name: "6 oz smoked catfish",
          image:
            "https://africanunique.com/cdn/shop/products/smokedcatfish_530x@2x.jpg?v=1609982165",
        },
        {
          name: "¾ cup palm oil",
          image:
            "https://www.supermart.ng/cdn/shop/files/omkt91106.jpg?v=1692003531",
        },
        {
          name: "1 roma tomato",
          image:
            "https://d2j6dbq0eux0bg.cloudfront.net/images/26774020/1363244751.jpg",
        },
        {
          name: "1 red bell pepper",
          image:
            "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
        },
        {
          name: "1 habanero pepper",
          image:
            "https://www.livingseedcompany.com/cdn/shop/products/habanero-pepper-capsicum-chinense-694660.png?v=1702656028",
        },
        {
          name: "1 cup stock",
          image:
            "https://media01.stockfood.com/largepreviews/MzQyMTQzNDY2/11036886-2-1-2-Cups-of-Chicken-Broth-in-a-Measuring-Cup.jpg",
        },
        {
          name: "½ onion",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "1 teaspoon cayenne pepper",
          image:
            "https://bonnieplants.com/cdn/shop/products/061422_T118630_202033_Bonnie_HotCayennePepper_ALT_02.jpg?v=1656010754&width=1000",
        },
        {
          name: "1 teaspoon crayfish",
          image:
            "https://housefood.africa/wp-content/uploads/2022/02/crayfish.jpg",
        },
        {
          name: "1 teaspoon bouillon powder",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "1 teaspoon salt",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
      ],
    },
    course: "Soups & Stews",
    prepTime: "5 minutes",
    cookTime: "25 minutes",
    totalTime: "30 minutes",
    prepLevel: "Easy",
    nutritionalFacts: {
      calories: 308,
      servings: 8,
      carbs: "8g",
      protein: "20g",
      fat: "21g",
      SaturatedFat: "4g",
      Cholesterol: "40mg",
      Sodium: "330mg",
      Potassium: "207mg",
      Fiber: "6g",
      Sugar: "1g",
      VitaminA: "542IU",
      VitaminC: "22mg",
      Calcium: "12mg",
      Iron: "1mg",
    },
    originalSource: "https://lowcarbafrica.com/bitter-leaf-soup/",
  },
  {
    id: 19,
    mealName: "Goat Meat Pepper Soup",
    overview:
      "This goat meat pepper soup is a favorite Nigerian soup that is so nutritious and filling. It's heartwarming, ethnic flavors will cheer you up and soothe your soul!",
    image:
      "https://i0.wp.com/www.goodmealtime.com/wp-content/uploads/2023/01/q4E0d6AsFPMtt23L.png?fit=1004%2C1008&ssl=1",

    recipe: {
      instructions: [
        "1. Cut the onions and set aside.",
        "2. Wash goat meat under running water and place it in a pot.",
        "3. Make it on the stovetop.",
        "4. Add 6 cups of water to the pot.",
        "5. Add the chopped onions, thyme, bouillon, and salt.",
        "6. Boil till tender (45 minutes to 1 hour).",
        "7. Add 4 more cups of water.",
        "8. Add pepper soup spice, cayenne pepper, and crayfish.",
        "9. Taste for salt and add more if needed.",
        "10. Simmer for 20 minutes on medium heat.",
        "11. Serve and enjoy!",
        "12. Make it in an instant pot.",
        "13. Place all the ingredients in your instant pot and add 8 cups of water.",
        "14. Set your instant pot to High Pressure for 40 minutes.",
        "15. Let the pressure naturally release when it's ready.",
      ],
      ingredients: [
        {
          name: "2 lbs goat meat",
          image:
            "https://www.excessrawfoods.com/wp-content/uploads/2019/11/goat-meat.jpeg",
        },
        {
          name: "½ onion",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "10 cups water",
          image:
            "https://img-cdn.inc.com/image/upload/w_1024,h_576,c_fill/images/panoramic/getty_1283524975_sgbp6n.jpg",
        },
        {
          name: "1 tablespoon salt",
          image:
            " https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
        {
          name: "1 teaspoon thyme",
          image:
            "https://rachelsspicecompany.com/wp-content/uploads/2018/03/ThymeLeaf.jpg",
        },
        {
          name: "1 teaspoon bouillon",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "2 tablespoons pepper soup spice",
          image:
            "https://www.gomarket.com.ng/wp-content/uploads/2020/05/Spice-Supreme-pepper-soup.jpg",
        },
        {
          name: "1 teaspoon cayenne pepper",
          image:
            "https://bonnieplants.com/cdn/shop/products/061422_T118630_202033_Bonnie_HotCayennePepper_ALT_02.jpg?v=1656010754&width=1000",
        },
        {
          name: "1 tablespoon ground crayfish",
          image:
            "https://housefood.africa/wp-content/uploads/2022/02/crayfish.jpg",
        },
      ],
    },
    course: "Soups & Stews",
    prepTime: "5 minutes",
    cookTime: "1 hour 20 minutes",
    totalTime: "1 hour 25 minutes",
    prepLevel: "Easy",
    nutritionalFacts: {
      calories: 154,
      servings: 8,
      carbs: "1g",
      protein: "29g",
      fat: "3g",
      Sodium: "887mg",
      Fiber: "1g",
      Sugar: "1g",
      VitaminC: "1mg",
      Calcium: "9mg",
    },
    originalSource: "https://lowcarbafrica.com/goat-meat-pepper-soup/",
  },
  {
    id: 20,
    mealName: "Cow Foot Soup (Cow Heel Soup)",
    overview:
      "This cow foot soup (cow heel soup) recipe is a low carb version of the popular cow foot soup enjoyed in Africa and the Caribbean. It is very rich and nutritious and comes with a host of benefits!",
    image: "https://africanbites.com/wp-content/uploads/2014/11/IMG_9489-2.jpg",

    recipe: {
      instructions: [
        "1. Chop the onions, then turn the instant pot to sauté mode.",
        "2. Add some oil to the instant pot and add the chopped onions.",
        "3. Let sauté for about a minute, then add about a half cup of water.",
        "4. Add the cow feet and the spices - bouillon powder, ground crayfish, smoked paprika, cayenne pepper, thyme, and salt.",
        "5. Stir till the cow feet is well coated with the spices.",
        "6. After about 2-3 minutes, pour in about 3-4 cups of water, or just enough to cover the cow feet.",
        "7. Place the instant pot lid back on and switch to manual mode. Set to 30 minutes on high pressure.",
        "8. Chop the red bell peppers and scallions.",
        "9. When the instant pot timer goes off, let it naturally release for 10 minutes before you quick-release the pressure.",
        "10. Turn the instant pot back to sauté mode.",
        "11. Add the chopped red bell pepper, and whole habanero pepper and let cook for about 5-10 minutes.",
        "12. Taste for salt and adjust if necessary.",
        "13. Combine 2 teaspoons of arrowroot powder and 1 tablespoon of water in a small bowl to make a slurry, and whisk together.",
        "14. Pour it into the cow foot soup and let it cook till it thickens.",
        "15. Garnish with scallions and serve!",
      ],
      ingredients: [
        {
          name: "3 lb cow feet",
          image:
            "https://atasteozimboivyskitchenette.files.wordpress.com/2016/10/img_9941.jpg",
        },
        {
          name: "1 onion, chopped",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "¼ cup olive oil",
          image:
            "https://content.instructables.com/F5R/Y0H0/IHVVPRS6/F5RY0H0IHVVPRS6.jpg?auto=webp&frame=1&width=320&md=a3d3b3b73ae688ac87e75fbee681be91",
        },
        {
          name: "2 teaspoons arrowroot powder",
          image:
            "https://skinfoodie.com.ng/wp-content/uploads/2019/11/Skin-food-arrow-root-powder.jpg",
        },
        {
          name: "1 habanero pepper",
          image:
            "https://www.livingseedcompany.com/cdn/shop/products/habanero-pepper-capsicum-chinense-694660.png?v=1702656028",
        },
        {
          name: "½ red bell pepper, chopped",
          image:
            "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
        },
        {
          name: "2 scallions, chopped",
          image:
            "https://upload.wikimedia.org/wikipedia/commons/d/da/CSA-Red-Spring-Onions.jpg",
        },
        {
          name: "1 teaspoon bouillon powder",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "1 teaspoon crayfish (substitute with coconut aminos)",
          image:
            "https://housefood.africa/wp-content/uploads/2022/02/crayfish.jpg",
        },
        {
          name: "1 teaspoon thyme",
          image:
            "https://rachelsspicecompany.com/wp-content/uploads/2018/03/ThymeLeaf.jpg",
        },
        {
          name: "1 teaspoon smoked paprika",
          image:
            "https://i0.wp.com/nextcashandcarry.com.ng/wp-content/uploads/2022/07/spspot2873.png?fit=512%2C512&ssl=1",
        },
        {
          name: "1 teaspoon cayenne pepper",
          image:
            "https://bonnieplants.com/cdn/shop/products/061422_T118630_202033_Bonnie_HotCayennePepper_ALT_02.jpg?v=1656010754&width=1000",
        },
        {
          name: "½ teaspoon salt (or to taste)",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
      ],
    },
    course: "Soups & Stews",
    prepTime: "5 minutes",
    cookTime: "1 hour 5 minutes",
    totalTime: "1 hour 10 minutes",
    prepLevel: "Easy",
    nutritionalFacts: {
      calories: 469,
      servings: 8,
      carbs: "1g",
      protein: "30g",
      fat: "1g",
      "Saturated Fat": "38g",
      Fiber: "1g",
      Sugar: "1g",
    },
    originalSource: "https://lowcarbafrica.com/cow-foot-soup/",
  },
  {
    id: 21,
    mealName: "Cauliflower Fufu (Swallow)",
    overview:
      "This African low carb, keto cauliflower fufu (swallow) recipe goes perfectly well with egusi and all your favorite soups. Just 10 net carbs!",
    image:
      "https://lowcarbafrica.com/wp-content/uploads/2018/04/African-Cauliflower-Fufu-swallow-homepage.jpg",

    recipe: {
      instructions: [
        "1. Cut one head of cauliflower into florets and wash.",
        "2. If using frozen cauliflower florets, microwave for a few minutes until tender.",
        "3. Put in blender and add water.",
        "4. Blend on high.",
        "5. Pour in cheesecloth and drain.",
        "6. Put in a pot on medium-low heat and stir.",
        "7. Add your psyllium husk and stir some more.",
        "8. If you want, add a tablespoon of palm oil and stir to your desired consistency.",
        "9. Refrigerate for one hour to harden up a bit.",
        "10. Serve with your soup of choice. Yum!",
      ],
      ingredients: [
        {
          name: "1 head cauliflower (medium) or 16oz frozen florets",
          image:
            "https://domf5oio6qrcr.cloudfront.net/medialibrary/5299/h1018g16207257715328.jpg",
        },
        {
          name: "2 tablespoons psyllium husk",
          image:
            "https://www-konga-com-res.cloudinary.com/w_400,f_auto,fl_lossy,dpr_3.0,q_auto/media/catalog/product/A/V/214680_1695236342.jpg",
        },
        {
          name: "1 tablespoon palm oil (optional)",
          image:
            "https://www.supermart.ng/cdn/shop/files/omkt91106.jpg?v=1692003531",
        },
      ],
    },
    course: "Main Course",
    prepTime: "15 minutes",
    cookTime: "10 minutes",
    totalTime: "25 minutes",
    prepLevel: "Easy",
    servings: 2,
    nutritionalFacts: {
      calories: 91,
      carbs: "19g",
      protein: "6g",
      fat: "1g",
      fiber: "10g",
      sugar: "6g",
    },
    originalSource: "https://lowcarbafrica.com/nigerian-cauliflower-fufu/",
  },
  {
    id: 22,
    mealName: "Coconut Fufu",
    overview:
      "Coconut Fufu is a low carb and nutritious African dish made from coconut flour. At only 12 net carbs, it is a delicious and healthy way to enjoy your favorite African soups!",
    image:
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQyDM0RoeIFbGSRHqyB517mL7sZtUqhNITC-pReTByF5_gDiV-uENW_4eC7WsjNRkV-Bfc&usqp=CAU",
    recipe: {
      instructions: [
        "1. Bring water to a boil in a pot.",
        "2. Mix the coconut fufu and psyllium husk together.",
        "3. Put mixture in boiling water and stir.",
        "4. Add more water if needed.",
        "5. Stir till it reaches a dough-like consistency.",
        "6. Serve with your favorite Nigerian soup.",
      ],
      ingredients: [
        {
          name: "1 cup coconut flour (sifted)",
          image:
            "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS8S-lTUJOBUReDMpIW2ganlYTWlrpCgoffSQ&usqp=CAU",
        },
        {
          name: "1.5 tablespoons psyllium husk",
          image:
            "https://www-konga-com-res.cloudinary.com/w_400,f_auto,fl_lossy,dpr_3.0,q_auto/media/catalog/product/A/V/214680_1695236342.jpg",
        },
        {
          name: "1.5 cups water",
          image:
            "https://img-cdn.inc.com/image/upload/w_1024,h_576,c_fill/images/panoramic/getty_1283524975_sgbp6n.jpg",
        },
      ],
    },
    course: "Main",
    prepTime: "5 minutes",
    cookTime: "5 minutes",
    totalTime: "10 minutes",
    prepLevel: "Easy",
    servings: 2,
    nutritionalFacts: {
      calories: 254,
      carbs: "35g",
      protein: "8g",
      fat: "8g",
      sodium: "9mg",
      fiber: "23g",
      sugar: "4g",
      calcium: "5mg",
    },
    originalSource: "https://lowcarbafrica.com/coconut-fufu/",
  },
  {
    id: 23,
    mealName: "Eggplant Amala Fufu (Swallow)",
    overview:
      "Eggplant Amala Fufu is the perfect low carb replacement for the traditional Nigerian Fufu dish, which is very high in carbs. Enjoy this healthy and spicy dish with your favorite Nigerian soup!",
    image:
      "https://lowcarbafrica.com/wp-content/uploads/2019/04/Eggplant-Amala.jpg",
    recipe: {
      instructions: [
        "1. Boil some water in a large pot.",
        "2. Cut the eggplants into large cubes.",
        "3. Boil the eggplants in hot water until very soft.",
        "4. Scoop the eggplants out of the boiled water into a large bowl lined with paper towels.",
        "5. Blend on high with a food processor until pureed.",
        "6. Pour the mixture in a pot and stir for 2 minutes on medium-high heat.",
        "7. Add the psyllium husk and stir for another 3 minutes.",
        "8. Serve with your favorite soup and enjoy!",
      ],
      ingredients: [
        {
          name: "2 Eggplants",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/279/279359/two-eggplants-on-a-wooden-table.jpg",
        },
        {
          name: "2 tablespoons Psyllium Husk",
          image:
            "https://www-konga-com-res.cloudinary.com/w_400,f_auto,fl_lossy,dpr_3.0,q_auto/media/catalog/product/A/V/214680_1695236342.jpg",
        },
      ],
    },
    course: "Main",
    prepTime: "5 minutes",
    cookTime: "20 minutes",
    totalTime: "25 minutes",
    prepLevel: "Easy",
    servings: 2,
    nutritionalFacts: {
      calories: 132,
      carbs: "30g",
      protein: "4g",
      fiber: "17g",
    },
    originalSource: "https://lowcarbafrica.com/eggplant-amala-fufu-swallow/",
  },
  {
    id: 24,
    mealName: "Cabbage Fufu",
    overview:
      "Cabbage Fufu (also known as cabbage swallow) is a wonderful, keto alternative to Nigerian (African) fufu dishes. It makes a delicious and healthy meal when paired with your favorite African soups!",
    image:
      "https://img-global.cpcdn.com/recipes/d8b524d13c454e4a/680x482cq70/cabbage-fufu-cabbage-swallow-recipe-main-photo.jpg",
    recipe: {
      instructions: [
        "1. Chop the cabbage into medium-sized pieces and blend with a blender.",
        "2. Blend until it’s completely pureed.",
        "3. Drape a large cheesecloth over a bowl, pour in the blended mix and drain out as much liquid as you can.",
        "4. Pour in a pan and place on the stove while stirring on medium-high heat.",
        "5. After about 2 minutes, add 2 tablespoons of psyllium husk.",
        "6. Stir continuously in a circular motion until it is completely blended in.",
        "7. Add one more tablespoon if you want it firmer.",
        "8. Keep stirring until the desired consistency is reached.",
        "9. Serve and enjoy!",
      ],
      ingredients: [
        {
          name: "1 green cabbage",
          image: "https://ladgroup.org/wp-content/uploads/2020/12/p5.jpg",
        },
        {
          name: "2-3 tablespoons Psyllium husk",
          image:
            "https://www-konga-com-res.cloudinary.com/w_400,f_auto,fl_lossy,dpr_3.0,q_auto/media/catalog/product/A/V/214680_1695236342.jpg",
        },
      ],
    },
    course: "Main Course",
    prepTime: "10 minutes",
    cookTime: "5 minutes",
    totalTime: "15 minutes",
    prepLevel: "Easy",
    servings: 2,
    nutritionalFacts: {
      calories: 131,
      carbs: "30g",
      protein: "5g",
      sodium: "81mg",
      potassium: "771mg",
      fiber: "15g",
      sugar: "14g",
      vitaminA: "445IU",
      vitaminC: "166.2mg",
      calcium: "182mg",
      iron: "2.1mg",
    },
    originalSource: "https://lowcarbafrica.com/cabbage-fufu/",
  },
  {
    id: 25,
    mealName: "Almond Flour Fufu",
    overview:
      "Almond flour fufu is a quick and easy keto fufu that makes an excellent alternative to high carb fufu like eba and pounded yam. Eat it with any African soup for a satisfying low carb meal!",
    image:
      "https://lowcarbafrica.com/wp-content/uploads/2020/08/Almond-Flour-Fufu-google-1.jpg",
    recipe: {
      instructions: [
        "1. Add one cup of water to a pot and turn on the heat.",
        "2. While waiting, mix the psyllium husk and almond flour in a bowl.",
        "3. When the water boils, pour in the flour and stir.",
        "4. Keep stirring till it begins to thicken.",
        "5. If after a minute, it is too thick, add very little water and stir some more.",
        "6. Wrap in a plastic wrap and let it sit for about 30 minutes if you want it firmer (optional).",
        "7. Serve and enjoy!",
      ],
      ingredients: [
        {
          name: "1 cup almond flour",
          image:
            "https://www-konga-com-res.cloudinary.com/w_400,f_auto,fl_lossy,dpr_3.0,q_auto/media/catalog/product/P/S/176171_1638814694.jpg",
        },
        {
          name: "1.5 tablespoons psyllium husk",
          image:
            "https://www-konga-com-res.cloudinary.com/w_400,f_auto,fl_lossy,dpr_3.0,q_auto/media/catalog/product/A/V/214680_1695236342.jpg",
        },
      ],
    },
    course: "Main",
    prepTime: "5 minutes",
    cookTime: "5 minutes",
    totalTime: "10 minutes",
    prepLevel: "Easy",
    servings: 1,
    nutritionalFacts: {
      calories: 660,
      carbs: "30g",
      protein: "24g",
      fat: "55g",
      saturatedFat: "4g",
      fiber: "18g",
      sugar: "4g",
      calcium: "235mg",
      iron: "4mg",
    },
    originalSource: "https://lowcarbafrica.com/almond-flour-fufu/",
  },
  {
    id: 26,
    mealName: "Snail Recipe (African Peppered Snails)",
    overview:
      "This delicious snail recipe (African peppered snails) sautéed with peppers, onions, and garlic, make a fantastic finger food or appetizer that will leave you licking your fingers!",
    image:
      "https://lowcarbafrica.com/wp-content/uploads/2023/08/African-Snail-Recipe-Peppered-Snails-IG.jpg",
    recipe: {
      instructions: [
        "1. To clean the snail:",
        "   a. If using fresh snails, boil them in a pot of water for 5 minutes.",
        "   b. Drain the water and using a pair of tweezers, pull the snails out of their shell, and place them in a bowl.",
        "   c. Squeeze in some lemon juice and let them sit for 5-10 minutes.",
        "   d. Scrub the snails with lemon until the sliminess is gone.",
        "   e. Rinse with plenty of cold water.",
        "2. To cook the snails:",
        "   a. Place snail in a pot, add water, 1 teaspoon bouillon, salt, and pepper.",
        "   b. Boil the snails for 20 -30 minutes till tender.",
        "   c. Alternatively, fry snails till crispy (about 10 minutes).",
        "3. To make the snail sauce:",
        "   a. Chop the habanero pepper, bell pepper, and onions, and mince the garlic.",
        "   b. Saute onions and garlic in olive oil till fragrant.",
        "   c. Add red bell pepper, habanero pepper, 1 teaspoon bouillon, cayenne pepper, and ground crayfish.",
        "   d. Saute for 5 mins.",
        "   e. Add snails, cover pot, and cook on medium-low heat for about 10 minutes.",
        "   f. About halfway, taste for salt and spices and adjust if necessary.",
        "4. Serve and enjoy!",
      ],
      ingredients: [
        {
          name: "1 lb snails, de-shelled",
          image:
            "https://247foods.ng/public/uploads/1656344893-h-250-0001-29557593827_20220627_164527_0000.png",
        },
        {
          name: "½ red bell peppers",
          image:
            "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
        },
        {
          name: "1 habanero pepper",
          image:
            "https://www.livingseedcompany.com/cdn/shop/products/habanero-pepper-capsicum-chinense-694660.png?v=1702656028",
        },
        {
          name: "½ onions",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "2 cloves garlic",
          image:
            "https://www.health.com/thmb/FAT5LSHXPSScbsi50u9EUw22rlo=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/Garlic-547276b9a356409eacef7284682b594a.jpg",
        },
        {
          name: "2 tablespoons olive oil",
          image:
            "https://content.instructables.com/F5R/Y0H0/IHVVPRS6/F5RY0H0IHVVPRS6.jpg?auto=webp&frame=1&width=320&md=a3d3b3b73ae688ac87e75fbee681be91",
        },
        {
          name: "1 teaspoon crayfish (substitute with 1 tablespoon of fish sauce)",
          image:
            "https://housefood.africa/wp-content/uploads/2022/02/crayfish.jpg",
        },
        {
          name: "2 teaspoons bouillon",
          image:
            " https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "1 teaspoon black pepper",
          image:
            "https://as1.ftcdn.net/v2/jpg/02/79/24/98/1000_F_279249862_iO0LUG8l16W0vvW3Aw16TYCxLuUwTAk2.jpg",
        },
        {
          name: "1 teaspoon salt (to cook)",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
        {
          name: "1 teaspoon cayenne pepper",
          image:
            "https://bonnieplants.com/cdn/shop/products/061422_T118630_202033_Bonnie_HotCayennePepper_ALT_02.jpg?v=1656010754&width=1000",
        },
        {
          name: "1 lemon (to clean the snail)",
          image:
            "https://upload.wikimedia.org/wikipedia/commons/thumb/f/f7/Lemon_-_whole_and_split.jpg/405px-Lemon_-_whole_and_split.jpg",
        },
      ],
    },
    course: "Snack, Side Dishes and Appetizers",
    prepTime: "10 minutes",
    cookTime: "35 minutes",
    totalTime: "45 minutes",
    prepLevel: "Easy",
    servings: 4,
    nutritionalFacts: {
      calories: 189,
      carbs: "8g",
      protein: "19g",
      fat: "9g",
      saturatedFat: "1g",
      cholesterol: "58mg",
      sodium: "92mg",
      potassium: "531mg",
      fiber: "2g",
      sugar: "2g",
      vitaminA: "790IU",
      vitaminC: "39mg",
      calcium: "24mg",
      iron: "4mg",
    },
    originalSource:
      "https://lowcarbafrica.com/peppered-snails-nigerian-snail-recipe/",
  },
  {
    id: 27,
    mealName: "Baked Whole Catfish",
    overview:
      "This Baked Whole Catfish is oven roasted and garnished with herbs and peppers for a tasty, healthy, low carb dish!",
    image:
      "https://thetopmeal.com/wp-content/uploads/2020/09/baked-whole-catfish-in-oven-5-720x720.jpg",
    recipe: {
      instructions: [
        "1. Preheat oven to 400.",
        "2. Blend the red bell pepper, habanero, and onions.",
        "3. Add the ginger, garlic, thyme, dill, sage, salt, and blend.",
        "4. Clean the fish using lemon to reduce the fishy smell.",
        "5. Make shallow slices on the fish, about 3 - 4 on both sides.",
        "6. Scoop about half of the pureed mixture into these holes and in the center slit.",
        "7. Keep the other half to use later.",
        "8. Completely wrap the fish in foil and place on the bottom rack of the oven for 45 mins.",
        "9. Optional: Add ¼ cup of olive oil to a pan on medium heat, then add the pureed mix and stir for 5 mins.",
        "10. Open the foil at the top and leave the fish in for another 5 minutes.",
        "11. Bring out the fish from the oven and garnish with the sauce or drizzle with olive oil if you don’t want to use the sauce.",
      ],
      ingredients: [
        {
          name: "1 catfish",
          image:
            "https://harvestselect.com/wp-content/uploads/2016/05/Harvest-Select-Catfish-Whole-Fish-2.jpg",
        },
        {
          name: "¼ cup olive oil",
          image:
            "https://content.instructables.com/F5R/Y0H0/IHVVPRS6/F5RY0H0IHVVPRS6.jpg?auto=webp&frame=1&width=320&md=a3d3b3b73ae688ac87e75fbee681be91",
        },
        {
          name: "½ red bell pepper",
          image:
            "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
        },
        {
          name: "½ onions, chopped",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "2 garlic cloves, minced",
          image:
            "https://www.health.com/thmb/FAT5LSHXPSScbsi50u9EUw22rlo=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/Garlic-547276b9a356409eacef7284682b594a.jpg",
        },
        {
          name: "½ teaspoon ginger root, chopped finely",
          image:
            "https://files.nccih.nih.gov/ginger-thinkstockphotos-531052216-square.jpg",
        },
        {
          name: "1 habanero pepper",
          image:
            "https://www.livingseedcompany.com/cdn/shop/products/habanero-pepper-capsicum-chinense-694660.png?v=1702656028",
        },
        {
          name: "½ sprig thyme leaves",
          image:
            "https://www.alphafoodie.com/wp-content/uploads/2021/02/How-to-Dry-Thyme-1-of-1.jpeg",
        },
        {
          name: "½ sprig dill",
          image:
            "https://scrummylane.com/wp-content/uploads/2022/09/fresh-dill.jpg",
        },
        {
          name: "1 sprig sage",
          image:
            "https://h2.commercev3.net/cdn.gurneys.com/images/800/70000A.jpg",
        },
      ],
    },
    course: "Main",
    prepTime: "30 minutes",
    cookTime: "50 minutes",
    totalTime: "1 hour 20 minutes",
    prepLevel: "Med",
    servings: 4,
    nutritionalFacts: {
      calories: 347,
      carbs: "3g",
      protein: "37g",
      fat: "19g",
      saturatedFat: "3g",
      cholesterol: "130mg",
      sodium: "98mg",
      potassium: "856mg",
      sugar: "1g",
      vitaminA: "600IU",
      vitaminC: "25.7mg",
      calcium: "37mg",
      iron: "0.8mg",
    },
    originalSource: "https://lowcarbafrica.com/spicy-oven-baked-catfish/",
  },
  {
    id: 28,
    mealName: "Instant Pot Boiled Peanuts",
    overview:
      "Enjoy these instant pot boiled peanuts as a healthy family snack during game day or movie nights. This spicy and salty snack is made with just a handful of spices, and using the instant pot is a time saver!",
    image:
      "https://southernbite.com/wp-content/uploads/2020/07/Instant-Pot-Boiled-Peanuts-3-500x500.jpg",
    recipe: {
      instructions: [
        "1. Rinse the peanuts and drain them in a colander.",
        "2. Place the peanuts in the instant pot and add your spices.",
        "3. Add enough water to cover them, but don't go over the maximum line.",
        "4. Place the instant pot trivet to weigh the peanuts down.",
        "5. Set your instant pot to manual mode and set it to high pressure for 70 minutes.",
        "6. When it's done, let the instant pot naturally release.",
        "7. Drain the peanuts in a colander and let them cool down a bit.",
      ],
      ingredients: [
        {
          name: "1 lb raw peanuts",
          image:
            "IMhttps://ed910ae2d60f0d25bcb8-80550f96b5feb12604f4f720bfefb46d.ssl.cf1.rackcdn.com/dd79402e574cd109-zm0UYLYi.jpg",
        },
        {
          name: "1 teaspoon crushed red pepper",
          image: "https://m.media-amazon.com/images/I/81UnXBtkE-L._SL1500_.jpg",
        },
        {
          name: "1 teaspoon cayenne pepper",
          image:
            "https://bonnieplants.com/cdn/shop/products/061422_T118630_202033_Bonnie_HotCayennePepper_ALT_02.jpg?v=1656010754&width=1000",
        },
        {
          name: "½ cup salt",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
        {
          name: "1 teaspoon garlic powder",
          image:
            "https://www.health.com/thmb/FAT5LSHXPSScbsi50u9EUw22rlo=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/Garlic-547276b9a356409eacef7284682b594a.jpg",
        },
      ],
    },
    course: "Appetizers and Side Dishes",
    prepTime: "5 minutes",
    cookTime: "1 hour 10 minutes",
    totalTime: "1 hour 15 minutes",
    prepLevel: "Easy",
    servings: 16,
    nutritionalFacts: {
      calories: 170,
      carbs: "8g",
      protein: "8g",
      fat: "14g",
      saturatedFat: "1g",
      polyunsaturatedFat: "1g",
      monounsaturatedFat: "1g",
      sodium: "362mg",
      potassium: "217mg",
      fiber: "3g",
      sugar: "1g",
      vitaminA: "37IU",
      vitaminC: "1mg",
      calcium: "3mg",
      iron: "1mg",
    },
    originalSource: "https://lowcarbafrica.com/instant-pot-boiled-peanuts/",
  },
  {
    id: 29,
    mealName: "Asun Recipe (Peppered Goat Meat)",
    overview:
      "Asun (Peppered Goat Meat) is a finger-licking, lip-smacking delicacy that is so delicious, you'll almost eat it all in one sitting!",
    image:
      "https://lowcarbafrica.com/wp-content/uploads/2019/09/Asun-recipe-IG-1.jpg",
    recipe: {
      instructions: [
        "1. Preheat your oven to 350.",
        "2. Cut the goat meat into small pieces, wash and set aside.",
        "3. Place goat meat in a large skillet and set the stove to medium heat.",
        "4. Add onion and bouillon powder and stir intermittently for 20 minutes.",
        "5. Line a baking pan with aluminum foil and spread the meat on one layer.",
        "6. Bake in the oven for 30 minutes, stirring halfway.",
        "7. While the goat meat is in the oven, chop onion, the green, yellow and red bell pepper, and habanero pepper.",
        "8. Preheat oil in a skillet and add the garlic and onion and stir for about a minute.",
        "9. Add the chopped peppers, bouillon powder, paprika, and black pepper and cook for about 5 minutes.",
        "10. Add the roasted goat meat and stir thoroughly. Allow it to simmer for about 5 minutes on medium heat.",
        "11. Add salt to taste if desired.",
        "12. Garnish with red pepper flakes, if desired.",
        "13. Serve and enjoy!",
      ],
      ingredients: [
        {
          name: "1 lb goat meat",
          image:
            "https://www.excessrawfoods.com/wp-content/uploads/2019/11/goat-meat.jpeg",
        },
        {
          name: "¼ onion",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "1 teaspoon bouillon powder",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "1 teaspoon red pepper flakes to garnish",
          image:
            "https://oldworldgardenfarms.com/wp-content/uploads/2018/09/featured-red-pepper-flakes.jpg",
        },
        {
          name: "salt to taste",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
        {
          name: "2 tablespoon olive oil",
          image:
            "https://content.instructables.com/F5R/Y0H0/IHVVPRS6/F5RY0H0IHVVPRS6.jpg?auto=webp&frame=1&width=320&md=a3d3b3b73ae688ac87e75fbee681be91",
        },
        {
          name: "1 cup green bell pepper chopped",
          image:
            "https://www.stylecraze.com/wp-content/uploads/2013/09/2267-Best-Benefits-Of-Green-Pepper-For-Skin-Hair-And-Health-is.jpg",
        },
        {
          name: "1 cup red bell pepper chopped",
          image:
            "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
        },
        {
          name: "1 cup yellow bell pepper chopped",
          image:
            "Ihttps://martking.ng/wp-content/uploads/2022/07/Martking-Online-Store-pepper.jpg",
        },
        {
          name: "1 habanero pepper chopped",
          image:
            "https://www.livingseedcompany.com/cdn/shop/products/habanero-pepper-capsicum-chinense-694660.png?v=1702656028",
        },
        {
          name: "1 teaspoon paprika",
          image:
            "https://i0.wp.com/nextcashandcarry.com.ng/wp-content/uploads/2022/07/spspot2873.png?fit=512%2C512&ssl=1",
        },
        {
          name: "1 teaspoon black pepper",
          image:
            "https://i5.walmartimages.com/asr/e13b1e9d-694c-4cbc-805a-0e7ff6099401.1935414d42c152ee37546a9d5c6a8edb.jpeg?odnHeight=768&odnWidth=768&odnBg=111111",
        },
        {
          name: "1 garlic clove",
          image:
            "https://www.health.com/thmb/FAT5LSHXPSScbsi50u9EUw22rlo=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/Garlic-547276b9a356409eacef7284682b594a.jpg",
        },
      ],
    },
    course: "Main",
    prepTime: "10 minutes",
    cookTime: "1 hour",
    totalTime: "1 hour 10 minutes",
    prepLevel: "Med",
    servings: 4,
    nutritionalFacts: {
      calories: 256,
      carbs: "8g",
      protein: "30g",
      fat: "10g",
      saturatedFat: "1g",
      sodium: "264mg",
      potassium: "255mg",
      fiber: "2g",
      sugar: "3g",
      vitaminA: "1685IU",
      vitaminC: "150.7mg",
      calcium: "10mg",
      iron: "0.6mg",
    },
    originalSource: "https://lowcarbafrica.com/asun-recipe-peppered-goat-meat/",
  },
  {
    id: 30,
    mealName: "Beef Liver Recipe",
    overview:
      "If you've never tried liver before, this classic beef liver recipe will make you fall in love with this superfood. Beef liver is one of the most nutritionally dense foods available, and calf liver topped with caramelized onions is both nutritious and delicious with a mild taste of liver.",
    image:
      "https://mommyshomecooking.com/wp-content/uploads/2018/02/Venezuelan-Liver-and-Onions-3.jpg",
    recipe: {
      instructions: [
        "1. If using beef liver, soak in milk for an hour or two, then pour the milk away.",
        "2. If liver is not sliced, slice about one inch thick.",
        "3. Wash and pat the liver dry.",
        "4. Mix spices together and liberally rub liver with spices.",
        "5. Chop onions into ring shapes by slicing lengthwise.",
        "6. Add 2 tablespoons of olive oil to a cast iron pan.",
        "7. When heated, add the onions and sprinkle a pinch of the mixed spices.",
        "8. Sauté the onions in olive oil for a few minutes till caramelized.",
        "9. Take the onions out and add 2 more tablespoons of olive oil to the pan.",
        "10. Add the liver pieces and cook liver for 3-5 mins on medium-high heat.",
        "11. Top with the onions and serve.",
      ],
      ingredients: [
        {
          name: "1 lb calf liver",
          image:
            "https://rawandfresh.com.au/cdn/shop/articles/liver_banner_image.png?v=1655942929&width=1100",
        },
        {
          name: "1 onion",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "4 tablespoon olive oil",
          image:
            "https://content.instructables.com/F5R/Y0H0/IHVVPRS6/F5RY0H0IHVVPRS6.jpg?auto=webp&frame=1&width=320&md=a3d3b3b73ae688ac87e75fbee681be91",
        },
        {
          name: "1 teaspoon garlic powder",
          image:
            "https://www.health.com/thmb/FAT5LSHXPSScbsi50u9EUw22rlo=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/Garlic-547276b9a356409eacef7284682b594a.jpg",
        },
        {
          name: "1 teaspoon onion powder",
          image:
            "https://www-konga-com-res.cloudinary.com/w_auto,f_auto,fl_lossy,dpr_auto,q_auto/media/catalog/product/E/F/192416_1636232818.jpgR",
        },
        {
          name: "1 teaspoon smoked paprika",
          image:
            "https://i0.wp.com/nextcashandcarry.com.ng/wp-content/uploads/2022/07/spspot2873.png?fit=512%2C512&ssl=1",
        },
        {
          name: "1 teaspoon black pepper",
          image:
            "https://as1.ftcdn.net/v2/jpg/02/79/24/98/1000_F_279249862_iO0LUG8l16W0vvW3Aw16TYCxLuUwTAk2.jpg",
        },
        {
          name: "1 teaspoon salt",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
      ],
    },
    course: "Main",
    prepTime: "5 minutes",
    cookTime: "10 minutes",
    totalTime: "15 minutes",
    prepLevel: "Easy",
    servings: 4,
    nutritionalFacts: {
      calories: 295,
      carbs: "6g",
      protein: "23g",
      fat: "20g",
      saturatedFat: "4g",
      polyunsaturatedFat: "2g",
      monounsaturatedFat: "11g",
      transFat: "1g",
      cholesterol: "379mg",
      sodium: "378mg",
      potassium: "354mg",
      fiber: "1g",
      sugar: "1g",
      vitaminA: "44290IU",
      vitaminC: "1mg",
      calcium: "8mg",
      iron: "7mg",
    },
    originalSource: "https://lowcarbafrica.com/liver-and-onions-recipe/",
  },
  {
    id: 31,
    mealName: "Peppered Gizzard",
    overview:
      "Peppered gizzard is a lip-smacking, mouthwatering delicacy that makes an excellent snack. If you have never tried gizzards before, this will make a believer out of you!",
    image:
      "https://cdn.shopify.com/s/files/1/0121/2950/1242/files/A3AFC3E2-C4E9-4BFA-A133-58ADD7F87262_480x480.jpg?v=1687170260",
    recipe: {
      instructions: [
        "1. Wash gizzards and boil for 15 minutes with some salt and black pepper.",
        "2. Coarsely blend the red bell pepper and habanero pepper.",
        "3. Chop the onions and scallions.",
        "4. When the gizzards are done, drain in a colander.",
        "5. Add frying oil to a large pan.",
        "6. When the oil gets hot, gently add the gizzards and let fry till crispy.",
        "7. Take out when done and place on paper towels.",
        "8. In a smaller pan, add ½ cup from the same frying oil and add chopped onions.",
        "9. When wilted, add the blended peppers and spices.",
        "10. Taste for salt and add some more if needed.",
        "11. Add in the gizzards and stir till well mixed in.",
        "12. Let simmer on low heat for 10 minutes.",
        "13. Add chopped scallions and serve.",
      ],
      ingredients: [
        {
          name: "1.5 lbs gizzard",
          image:
            "https://cdn11.bigcommerce.com/s-e132r1196t/images/stencil/1280x1280/products/1762/2426/gizzard_raw__96111.1559233089.1280.1280__66331.1585176209.1280.1280__59050.1589852643.jpg?c=2",
        },
        {
          name: "½ cup olive oil",
          image:
            "https://content.instructables.com/F5R/Y0H0/IHVVPRS6/F5RY0H0IHVVPRS6.jpg?auto=webp&frame=1&width=320&md=a3d3b3b73ae688ac87e75fbee681be91",
        },
        {
          name: "1 red bell pepper",
          image:
            "https://i5.walmartimages.com/seo/Fresh-Red-Bell-Pepper-1-Each_7be94a8e-9a5d-4f87-842f-5fe4084138ba.c95d36e140f5e0d492ca632b42e4543c.jpeg",
        },
        {
          name: "1 habanero pepper or use ½",
          image:
            "https://www.livingseedcompany.com/cdn/shop/products/habanero-pepper-capsicum-chinense-694660.png?v=1702656028",
        },
        {
          name: "1 cup onions chopped",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
        {
          name: "1 teaspoon bouillon powder",
          image:
            "https://www.mapleandmango.com/wp-content/uploads/2021/09/bouillon-powder-in-jar.jpg",
        },
        {
          name: "½ teaspoon thyme",
          image:
            "https://rachelsspicecompany.com/wp-content/uploads/2018/03/ThymeLeaf.jpg",
        },
        {
          name: "½ teaspoon black pepper",
          image:
            "https://as1.ftcdn.net/v2/jpg/02/79/24/98/1000_F_279249862_iO0LUG8l16W0vvW3Aw16TYCxLuUwTAk2.jpg",
        },
        {
          name: "1/4 teaspoon cayenne pepper omit to reduce the heat",
          image:
            "https://bonnieplants.com/cdn/shop/products/061422_T118630_202033_Bonnie_HotCayennePepper_ALT_02.jpg?v=1656010754&width=1000",
        },
        {
          name: "½ teaspoon salt or to taste",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
        {
          name: "scallions optional garnish",
          image:
            "https://upload.wikimedia.org/wikipedia/commons/d/da/CSA-Red-Spring-Onions.jpg",
        },
        {
          name: "more olive oil for frying the gizzard",
          image:
            "https://content.instructables.com/F5R/Y0H0/IHVVPRS6/F5RY0H0IHVVPRS6.jpg?auto=webp&frame=1&width=320&md=a3d3b3b73ae688ac87e75fbee681be91",
        },
      ],
    },
    course: "Snack, Side Dishes and Appetizers",
    prepTime: "5 minutes",
    cookTime: "35 minutes",
    totalTime: "40 minutes",
    prepLevel: "Med",
    servings: 8,
    nutritionalFacts: {
      calories: 211,
      carbs: "2g",
      protein: "15g",
      fat: "15g",
      saturatedFat: "2g",
      cholesterol: "1mg",
      sodium: "66mg",
      potassium: "31mg",
      fiber: "1g",
      sugar: "1g",
      vitaminA: "478IU",
      vitaminC: "21mg",
      iron: "1mg",
    },
    originalSource: "https://lowcarbafrica.com/peppered-gizzard/",
  },
  {
    id: 32,
    mealName: "Nigerian Native Jollof Rice (Keto Version)",
    overview:
      "Enjoy this delicious, low carb, cauliflower palm oil rice, spiced with mouthwatering African spices. Just 1g net carb!",
    image:
      "https://lowcarbafrica.com/wp-content/uploads/2019/03/Cauliflower-Local-Jollof-Rice-IG-1.jpg",
    recipe: {
      instructions: [
        "1. If using smoked catfish and Iru, soak in hot water for 5 minutes.",
        "2. Rinse out the Iru and drain.",
        "3. Break the fish into smaller chunks and remove the pieces of bone.",
        "4. In a large skillet, add the palm oil.",
        "5. When it gets hot, add the chopped onions, dry fish, and Iru and stir on medium heat for about 5 minutes.",
        "6. Add your cauliflower rice and keep stirring till well saturated.",
        "7. Add salt, pepper, and crayfish to taste (and other spices if you desire).",
        "8. Cook on medium heat for another 10 minutes.",
        "9. Serve & Enjoy.",
      ],
      ingredients: [
        {
          name: "1 riced cauliflower",
          image:
            "https://domf5oio6qrcr.cloudfront.net/medialibrary/5299/h1018g16207257715328.jpg",
        },
        {
          name: "¼ cup palm oil",
          image:
            "https://www.supermart.ng/cdn/shop/files/omkt91106.jpg?v=1692003531",
        },
        {
          name: "2 tablespoon crayfish",
          image:
            "https://housefood.africa/wp-content/uploads/2022/02/crayfish.jpg",
        },
        {
          name: "10 pieces Iru locust bean seeds",
          image:
            "https://guardian.ng/wp-content/uploads/2016/01/dawadawa-Locust-Beans1.jpg",
        },
        {
          name: "1 Smoked catfish",
          image:
            "https://africanunique.com/cdn/shop/products/smokedcatfish_530x@2x.jpg?v=1609982165",
        },
        {
          name: "½ onion",
          image:
            "https://as2.ftcdn.net/v2/jpg/02/94/20/65/1000_F_294206555_6brCSze7hgSF7Fcx4irETGJCgt3oxFBt.jpg",
        },
      ],
    },
    course: "Main",
    prepTime: "5 minutes",
    cookTime: "15 minutes",
    totalTime: "20 minutes",
    prepLevel: "Med",
    servings: 4,
    nutritionalFacts: {
      calories: 371,
      carbs: "3g",
      protein: "40g",
      fat: "19g",
      saturatedFat: "1g",
      cholesterol: "140mg",
      sodium: "104mg",
      potassium: "843mg",
      fiber: "2g",
      vitaminA: "115IU",
      vitaminC: "2.6mg",
      calcium: "38mg",
      iron: "0.8mg",
    },
    originalSource:
      "https://lowcarbafrica.com/nigerian-cauliflower-palm-oil-rice/",
  },
  {
    id: 33,
    mealName: "Keto Avocado Smoothie",
    overview:
      "This creamy and silky smooth keto avocado smoothie makes an excellent breakfast or midday snack. It is gluten-free, dairy-free and sugar-free!",
    image:
      "https://thebigmansworld.com/wp-content/uploads/2020/12/keto-avocado-smoothie4.jpg",
    recipe: {
      instructions: [
        "1. Add all the ingredients to a high powered blender.",
        "2. Blend till it is smooth and creamy.",
        "3. Serve and enjoy!",
      ],
      ingredients: [
        {
          name: "1 avocado",
          image:
            "https://media.post.rvohealth.io/wp-content/uploads/sites/3/2021/07/Stocksy_txpa387b205vt9300_Medium_3358820_thumb-732x549.jpg",
        },
        {
          name: "1 teaspoon vanilla extract",
          image:
            "https://www.rodellekitchen.com/wp-content/uploads/2018/04/08598122804-1-scaled.webp",
        },
        {
          name: "1 -2 tablespoon monk fruit sweetener",
          image:
            "https://m.media-amazon.com/images/W/MEDIAX_792452-T2/images/I/711bpHe4RgL._SL1280_.jpg",
        },
        {
          name: "1 cup almond milk",
          image:
            "https://navs-online.org/wp-content/uploads/2016/03/Almond-Milk-v2.jpg",
        },
        {
          name: "1 tablespoon peanut butter",
          image:
            "https://img.livestrong.com/-/clsd/getty/650fceadf35548cb89ad2b3c164ba36c.jpg",
        },
        {
          name: "A handful of ice",
          image:
            "https://cdn.pixabay.com/photo/2021/04/28/23/58/ice-6215207_640.jpg",
        },
      ],
    },
    course: "Breakfast",
    prepTime: "5 minutes",
    totalTime: "5 minutes",
    prepLevel: "Easy",
    servings: 1,
    nutritionalFacts: {
      calories: 452,
      carbs: "21g",
      protein: "9g",
      fat: "41g",
      saturatedFat: "4g",
      sodium: "74mg",
      potassium: "1070mg",
      fiber: "16g",
      sugar: "2g",
      vitaminA: "303IU",
      vitaminC: "20mg",
      calcium: "70mg",
      iron: "5mg",
    },
    originalSource:
      "https://lowcarbafrica.com/keto-avocado-smoothie-dairy-free/",
  },
  {
    id: 34,
    mealName: "Strawberry Spinach Smoothie",
    overview:
      "Start your day right with this creamy strawberry spinach smoothie recipe. This healthy smoothie takes a minute to throw together and makes a quick keto breakfast or perfect snack!",
    image:
      "https://i1.wp.com/thewimpyvegetarian.com/wp-content/uploads/2022/06/Spinach-and-Strawberry-Smoothie-1.jpg",
    recipe: {
      instructions: [
        "1. Add the almond milk and heavy whipping cream to a blender or nutribullet.",
        "2. Add the frozen strawberries, vanilla extract, and monk fruit sweetener.",
        "3. Add the spinach last.",
        "4. Blend until smooth.",
      ],
      ingredients: [
        {
          name: "¾ cup frozen strawberries",
          image:
            "https://www.foodandwine.com/thmb/tfqE6pWWvkpYafhY1u91LE6yRtY=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/Frozen-Strawberries-FT-BLOG0623-729a216a8456465d92c4ecf4888b6a1f.jpgS",
        },
        {
          name: "1 cup spinach",
          image:
            "https://hips.hearstapps.com/hmg-prod/images/spinach-1521576635.jpg",
        },
        {
          name: "1 cup almond milk",
          image:
            "https://navs-online.org/wp-content/uploads/2016/03/Almond-Milk-v2.jpg",
        },
        {
          name: "¼ cup heavy cream",
          image:
            "https://ketopots.com/wp-content/uploads/2020/02/5a-keto-whipped-cream.jpg",
        },
        {
          name: "1 teaspoon vanilla extract",
          image:
            "https://www.rodellekitchen.com/wp-content/uploads/2018/04/08598122804-1-scaled.webp",
        },
        {
          name: "1 tablespoon monk fruit sweetener",
          image:
            "https://m.media-amazon.com/images/W/MEDIAX_792452-T2/images/I/711bpHe4RgL._SL1280_.jpg",
        },
      ],
    },
    course: "Breakfast",
    prepTime: "1 minute",
    totalTime: "2 minutes",
    prepLevel: "Easy",
    servings: 1,
    nutritionalFacts: {
      calories: 272,
      carbs: "11g",
      protein: "2g",
      fat: "27g",
      saturatedFat: "1g",
      sodium: "24mg",
      potassium: "167mg",
      fiber: "4g",
      sugar: "1g",
      vitaminA: "2823IU",
      vitaminC: "18mg",
      calcium: "75mg",
      iron: "5mg",
    },
    originalSource: "https://lowcarbafrica.com/strawberry-spinach-smoothie/",
  },
  {
    id: 35,
    mealName: "Keto Chocolate Protein Shake",
    overview:
      "This keto chocolate protein shake makes a refreshing breakfast or dessert that is nutritious, filling, and keeps hunger at bay! Plus, it has a whopping 31 grams of protein and is a great way to fuel your day on a keto diet.",
    image:
      "https://lowcarbyum.com/wp-content/uploads/2020/09/keto-frosty-chocolate-shake-recipe-l.jpg",
    recipe: {
      instructions: [
        "1. Add the almond milk first, then add the other ingredients.",
        "2. Add some ice, then blend on high in a blender or nutribullet.",
        "3. Top up with zero-carb whipped cream, cinnamon, or shaved chocolate.",
      ],
      ingredients: [
        {
          name: "1 scoop protein powder (approx 2 tbsp)",
          image:
            "https://ng.jumia.is/unsafe/fit-in/500x500/filters:fill(white)/product/67/5766032/1.jpg?0069",
        },
        {
          name: "1 tablespoon cacao powder",
          image:
            "https://image.makewebeasy.net/makeweb/m_1920x0/PwajjYvAA/228150/1.jpg",
        },
        {
          name: "1 teaspoon vanilla extract",
          image:
            "https://www.rodellekitchen.com/wp-content/uploads/2018/04/08598122804-1-scaled.webp",
        },
        {
          name: "2 tablespoon monk fruit sweetener",
          image:
            "https://m.media-amazon.com/images/W/MEDIAX_792452-T2/images/I/711bpHe4RgL._SL1280_.jpg",
        },
        {
          name: "1 cup almond milk (unsweetened)",
          image:
            "https://navs-online.org/wp-content/uploads/2016/03/Almond-Milk-v2.jpg",
        },
        {
          name: "¼ cup heavy whipping cream",
          image:
            "https://ketopots.com/wp-content/uploads/2020/02/5a-keto-whipped-cream.jpg",
        },
        {
          name: "1 tablespoon almond butter",
          image:
            "https://hungryhealthyhappy.com/wp-content/uploads/2018/02/Vanilla-and-Honey-Almond-Butter-featured.jpg",
        },
        {
          name: "pinch of salt",
          image:
            "https://cdn-prod.medicalnewstoday.com/content/images/articles/322/322745/salt-shaker.jpg",
        },
      ],
    },
    course: "Desserts",
    prepTime: "1 minute",
    totalTime: "2 minutes",
    prepLevel: "Easy",
    servings: 1,
    nutritionalFacts: {
      calories: 445,
      carbs: "7g",
      protein: "31g",
      fat: "12g",
      saturatedFat: "24g",
      sodium: "230mg",
      potassium: "115mg",
      fiber: "3g",
      sugar: "1g",
      vitaminA: "10IU",
      calcium: "47mg",
      iron: "4mg",
    },
    originalSource:
      "https://lowcarbafrica.com/keto-chocolate-almond-protein-shake/",
  },
  {
    id: 36,
    mealName: "Low Carb Lemon Smoothie",
    overview:
      "This low carb lemon smoothie is like sunshine in a glass! It's the perfect drink for the summer and is sugar-free, LCHF, and keto-friendly.",
    image:
      "https://stepawayfromthecarbs.com/wp-content/uploads/2019/05/low-carb-lemon-smoothie-3-480x360.jpg",
    recipe: {
      instructions: [
        "1. Add the low carb vanilla yogurt to a blender, together with the ice cubes, the zest and juice of one lemon, heavy cream, and turmeric.",
        "2. Blend until smooth, then taste for sweetness - you might like the sharp lemon taste, but if you prefer it sweeter then add some homemade simple syrup, liquid stevia, or even some low carb honey.",
        "3. Pour into a glass and enjoy the taste of sunshine - low carb style!",
      ],
      ingredients: [
        {
          name: "1 cup Two Good Vanilla Yogurt",
          image:
            "https://farmfreshng.com/wp-content/uploads/2020/05/Farmfresh-Yoghurt-Vanilla-500ml.jpg",
        },
        {
          name: "½ cup ice",
          image:
            "https://cdn.pixabay.com/photo/2021/04/28/23/58/ice-6215207_640.jpg",
        },
        {
          name: "¼ cup heavy cream",
          image:
            "https://ketopots.com/wp-content/uploads/2020/02/5a-keto-whipped-cream.jpg",
        },
        {
          name: "zest and juice of 1 lemon",
          image:
            "https://upload.wikimedia.org/wikipedia/commons/thumb/f/f7/Lemon_-_whole_and_split.jpg/405px-Lemon_-_whole_and_split.jpg",
        },
        {
          name: "⅛ tsp turmeric",
          image:
            "https://staticcookist.akamaized.net/wp-content/uploads/sites/22/2018/10/turmeric.jpg",
        },
        {
          name: "1 tbs low carb simple syrup (optional)",
          image:
            "https://www.u-buy.com.ng/productimg/?image=aHR0cHM6Ly9tLm1lZGlhLWFtYXpvbi5jb20vaW1hZ2VzL0kvNjFNVUxBSFZvTEwuX1NMMTUwMF8uanBn.jpg",
        },
      ],
    },
    course: "Beverage",
    prepTime: "5 minutes",
    totalTime: "5 minutes",
    prepLevel: "Easy",
    servings: 2,
    nutritionalFacts: {
      calories: 146,
      totalFat: "12g",
      saturatedFat: "7g",
      transFat: "1g",
      cholesterol: "43mg",
      sodium: "32mg",
      carbohydrates: "4g",
      netCarbohydrates: "3g",
      fiber: "1g",
      sugar: "1g",
      protein: "7g",
    },
    originalSource: "https://stepawayfromthecarbs.com/low-carb-lemon-smoothie/",
  },
  //   {
  //     id: 37,
  //     mealName: "Green Detox Keto Smoothie",
  //     overview:
  //       "Revitalize your day with this Green Detox Keto Smoothie. Packed with the goodness of kale, fresh mint, avocado, fennel, and more, this unsweetened, low-carb smoothie is a refreshing treat for your body and taste buds.",
  //     image:
  //       "https://www.sugarfreemom.com/wp-content/uploads/2016/06/green-tea-detox-smoothie1-1-of-1.jpg",
  //     recipe: {
  //       instructions: [
  //         "1. Scoop the avocado flesh out and place all ingredients into a high-speed blender.",
  //         "2. Blend until smooth.",
  //         "3. Adjust thickness by adding more almond milk if desired.",
  //       ],
  //       ingredients: [
  //         {
  //           name: "Handful / 15g kale",
  //           image:
  //             "https://www.shutterstock.com/image-photo/bunch-organic-kale-on-rustic-600nw-291502316.jpg",
  //         },
  //         {
  //           name: "Handful / 10g fresh mint leaves, picked",
  //           image:
  //             "https://plantura.garden/uk/wp-content/uploads/sites/2/2021/04/store-fresh-mint.jpg",
  //         },
  //         {
  //           name: "½ avocado",
  //           image:
  //             "https://media.post.rvohealth.io/wp-content/uploads/sites/3/2021/07/Stocksy_txpa387b205vt9300_Medium_3358820_thumb-732x549.jpg",
  //         },
  //         {
  //           name: "¼ / 50g fennel or 1 celery stick",
  //           image:
  //             "https://www.dadcooksdinner.com/wp-content/uploads/2020/03/Stalk-of-Celery-vs-Rib-of-Celery-2020-03-03-Celery-Stalk-vs-Rib0391-1920x.jpg",
  //         },
  //         {
  //           name: "¼ small cucumber (100g)",
  //           image:
  //             "https://sa1s3optim.patientpop.com/assets/images/provider/photos/2445929.png",
  //         },
  //         {
  //           name: "1 tablespoon protein powder unsweetened",
  //           image:
  //             "https://ng.jumia.is/unsafe/fit-in/500x500/filters:fill(white)/product/67/5766032/1.jpg?0069",
  //         },
  //         {
  //           name: "1 cup / 240 ml almond milk unsweetened",
  //           image:
  //             "https://navs-online.org/wp-content/uploads/2016/03/Almond-Milk-v2.jpg",
  //         },
  //         {
  //           name: "1 tablespoon almond butter",
  //           image:
  //             "https://hungryhealthyhappy.com/wp-content/uploads/2018/02/Vanilla-and-Honey-Almond-Butter-featured.jpg",
  //         },
  //         {
  //           name: "½ lemon, juiced",
  //           image:
  //             "https://hungryhealthyhappy.com/wp-content/uploads/2018/02/Vanilla-and-Honey-Almond-Butter-featured.jpg",
  //         },
  //       ],
  //     },
  //     course: "Beverage",
  //     prepTime: "5 minutes",
  //     totalTime: "5 minutes",
  //     prepLevel: "Easy",
  //     servings: 1,
  //     nutritionalFacts: {
  //       calories: 137,
  //       totalCarbohydrates: "7.7g",
  //       protein: "5.6g",
  //       fat: "10g",
  //       fiber: "4.7g",
  //       sugar: "1.7g",
  //     },
  //     originalSource: "https://sugarfreelondoner.com/keto-green-smoothie/",
  //   },
  {
    id: 38,
    mealName: "Keto Avocado Chocolate Smoothie",
    overview:
      "Indulge in the creamy goodness of this Keto Avocado Chocolate Smoothie, a perfect blend of healthy fats and protein. Whether for breakfast or as a snack, this vegan and keto-friendly smoothie is a delightful treat with just 5 simple ingredients - almond milk, nut butter, cocoa powder, sweetener, and avocado.",
    image:
      "https://www.peelwithzeal.com/wp-content/uploads/2021/01/chocolate-avocado-smoothie-vegan.jpg",
    recipe: {
      instructions: [
        "1. Add all ingredients to a high-powered blender and mix.",
        "2. For a thicker smoothie, add less almond milk.",
        "3. Adjust stevia to taste.",
        "4. Garnish with sugar-free chocolate chips, if desired.",
      ],
      ingredients: [
        {
          name: "Almond milk",
          image:
            "https://navs-online.org/wp-content/uploads/2016/03/Almond-Milk-v2.jpg",
        },
        {
          name: "Almond butter",
          image:
            "https://hungryhealthyhappy.com/wp-content/uploads/2018/02/Vanilla-and-Honey-Almond-Butter-featured.jpg",
        },
        {
          name: "Avocado",
          image:
            "https://hungryhealthyhappy.com/wp-content/uploads/2018/02/Vanilla-and-Honey-Almond-Butter-featured.jpg",
        },
        {
          name: "Unsweetened cocoa powder",
          image:
            "https://image.makewebeasy.net/makeweb/m_1920x0/PwajjYvAA/228150/1.jpg",
        },
        {
          name: "Liquid stevia",
          image:
            "https://images-cdn.ubuy.co.in/650342635574e60d41500149-organic-liquid-stevia-sweetener-alcohol.jpg",
        },
      ],
    },
    course: "Beverage",
    prepTime: "5 minutes",
    totalTime: "5 minutes",
    prepLevel: "Easy",
    servings: 1,
    nutritionalFacts: {
      calories: 305,
      totalCarbohydrates: "4g",
      protein: "9g",
      fat: "10g",
      fiber: "3g",
      sugar: "1g",
    },
    originalSource:
      "https://www.seasonalcravings.com/keto-avocado-chocolate-smoothie/#nutrition",
  },

  // Add more recipes following the same structure
];

export default recipes;
